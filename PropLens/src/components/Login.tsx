 ...
 * work in production — PropLens runs LOCAL_MODE=1 with no Firebase credentials,
 ...
 * code issued per agency.
 *
 * ── The dead end this screen used to be ────────────────────────────────────
 * The code is checked by POST /api/auth/device, and the first thing that route
 * does is refuse everything with 503 when neither PROPLENS_ACCESS_CODE nor
 * PROPLENS_AGENCY_CODES is set. On an instance where nobody has set either —
...
export default function Login() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // were given, rather than being told the site is misconfigured.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!deviceCode.trim()) {
      setCodeError('Enter the access code for this agency.');
      return;
    }
    setCodeBusy(true);
    setCodeError(null);
    ...
    >
      ...
      <div className="w-full max-w-sm z-10 flex flex-col items-center">
        ...
        <div className="text-center mb-10">
          ...
          />
          <p className="mt-3 text-[12px] text-[rgba(10,20,32,0.55)] tracking-[0.08em]">
            Property photo studio
          </p>
        </div>
        ...
        <div className="text-center mb-7">
          ...
          <p className="mt-1.5 text-[13px] text-[rgba(10,20,32,0.55)]">
        ...
        {server === 'unconfigured' ? (
          ...
          <div className="w-full space-y-4">
            ...
            <div className="rounded-2xl border border-[rgba(10,20,32,0.08)] bg-[rgba(255,255,255,0.85)] p-4">
              <div className="flex items-start gap-2.5">
                <ShieldOff size={16} className="text-[rgba(10,20,32,0.55)] shrink-0 mt-0.5" />
                <div className="min-w-0">
                  <p className="text-[13px] font-semibold text-[#0A1420]">
                    No access code is set on this server
                  </p>
                  <p className="mt-1 text-[13px] text-[rgba(10,20,32,0.72)] leading-relaxed">
                    Until one is, sign-in codes cannot be checked and this instance
                    holds local data only.
                  </p>
                  <p className="mt-2 text-[12px] font-mono text-[rgba(10,20,32,0.55)] break-all">
                    PROPLENS_ACCESS_CODE
                  </p>
                </div>
              </div>
            </div>
            ...
            <p className="text-[13px] text-[rgba(10,20,32,0.55)] text-center leading-relaxed">
          ...
          <form onSubmit={handleSubmit} className="w-full space-y-4">
            <div className="space-y-2">
              ...
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-[rgba(10,20,32,0.42)] group-focus-within:text-[#0E9D98] transition-colors">
                  <Lock size={15} />
                </div>
                ...
                <input
                  ...
                  className="block w-full pl-11 pr-14 h-[52px] bg-[rgba(255,255,255,0.85)] border border-[rgba(10,20,32,0.10)] rounded-xl shadow-[inset_0_2px_4px_rgba(10,20,32,0.06)] text-[16px] text-[#0A1420] placeholder-[rgba(10,20,32,0.32)] focus:outline-none focus:border-[#0E9D98]/60 focus:bg-white transition-all"
                />
                <button
                  type="button"
                  ...
                  aria-pressed={reveal}
                  className="absolute right-1 top-1/2 -translate-y-1/2 h-11 w-11 grid place-items-center rounded-lg text-[rgba(10,20,32,0.55)] hover:text-[#0A1420] transition-colors"
                >
                  {reveal ? <EyeOff size={16} /> : <Eye size={16} />}
                </button>
              </div>
              {capsOn && (
                <p className="text-[13px] text-[rgba(10,20,32,0.55)] ml-1">Caps Lock is on.</p>
              )}
            </div>
            ...
            {codeError && (
              <div className="bg-red-50 border border-red-200 p-3 rounded-xl flex items-start gap-2">
                ...
                <p className="text-[13px] font-medium text-red-700 leading-snug">{codeError}</p>
              </div>
            )}
        ...
        {server !== 'unconfigured' && (
          <p className="mt-6 text-[13px] text-[rgba(10,20,32,0.55)] text-center leading-relaxed">
        ...
        <p className="mt-10 text-center text-[12px] text-[rgba(10,20,32,0.55)]">
          <a
            href="https://tru-property.co.za"
            target="_blank"
            rel="noopener noreferrer"
            className="text-[rgba(10,20,32,0.72)] hover:text-[#0E9D98] transition-colors"
          >
            PropLens
          </a>
