...
const TYPES: DamageFinding['damageType'][] = [
  'scratch', 'dent', 'chip', 'rust', 'crack', 'paint', 'wear', 'missing', 'other',
];

const SEVERITY_META: Record<number, { label: string; color: string }> = {
  1: { label: 'Cosmetic', color: '#0E9D98' },
    ...
    <div className="flex flex-col h-full bg-[#F5F4F1] text-[#0A1420] overflow-hidden">
      ...
      <div className="bg-[#0A1420]/80 backdrop-blur-xl border-b border-white/[0.06] p-4 flex items-center justify-between shrink-0">
        ...
        <div className="text-right shrink-0">
          ...
          <div className="text-[13px] text-white/50">across {shotSlots.length} photos</div>
        </div>
      </div>

      {/* Photo picker */}
      <div className="flex gap-2 overflow-x-auto p-3 border-b border-[rgba(10,20,32,0.10)] shrink-0">
        {shotSlots.map((s) => {
      ...
      <div className="flex-1 overflow-y-auto">
        <div
          ...
          className="relative w-full bg-[#F5F4F1] select-none cursor-crosshair"
        >
          ...
          {!slotTags.length && (
            <div className="absolute inset-x-0 bottom-0 p-2 text-center text-[13px] text-white bg-black/50">
        ...
        {editing && (
          <div className="p-4 space-y-3 border-t border-[rgba(10,20,32,0.10)]">
            <div className="flex items-center justify-between">
              <span className="text-[13px] font-bold text-[#0A1420] flex items-center gap-2">
                Mark on {editing.panel}
                {editing.source === 'ai' && !editing.confirmed && (
                  <span className="text-[13px] font-bold text-[#0E9D98] border border-[#0E9D98]/40 rounded px-2 py-0.5 flex items-center gap-1">
              ...
              <button
                ...
                className="flex items-center gap-1 text-[13px] text-[#C07676] hover:text-red-400"
              >
            ...
            <div>
              <label className="text-[13px] text-[rgba(10,20,32,0.55)] font-bold block mb-2">Type</label>
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded-full text-[13px] font-semibold border capitalize ${
                      editing.damageType === t
                        ? 'bg-[#0E9D98]/15 border-[#0E9D98]/50 text-[#0E9D98]'
            ...
            <div>
              <label className="text-[13px] text-[rgba(10,20,32,0.55)] font-bold block mb-2">Severity</label>
                ...
                {[1, 2, 3, 4, 5].map((n) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg text-[13px] font-bold border ${active ? '' : 'bg-white border-[rgba(10,20,32,0.10)] text-[rgba(10,20,32,0.55)]'}`}
                    >
            ...
            <div>
              <label className="text-[13px] text-[rgba(10,20,32,0.55)] font-bold block mb-2">Note (what / where / size)</label>
              <input
                type="text"
                value={editing.note}
                onChange={(e) => updateTag(editing.id, { note: e.target.value })}
                placeholder="e.g. 15cm scratch through clearcoat, lower door"
                className="w-full px-3 py-2 bg-white border border-[rgba(10,20,32,0.10)] rounded-lg text-[13px] text-[#0A1420] placeholder-[rgba(10,20,32,0.35)] focus:outline-none focus:border-[#0E9D98]/50"
              />
            </div>
            ...
            {editing.source === 'ai' && !editing.confirmed ? (
              <button
                ...
                className="w-full py-3 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-[13px] font-semibold text-white flex items-center justify-center gap-2"
              >
              ...
              <button
                ...
                className="w-full py-2 rounded-lg bg-white border border-[rgba(10,20,32,0.10)] text-[13px] font-semibold text-[#0A1420] flex items-center justify-center gap-2"
              >
        ...
        {!editing && (
          <p className="px-4 py-3 text-[13px] text-[rgba(10,20,32,0.55)] flex items-center gap-2">
      ...
      {/* Save bar */}
      <div className="shrink-0 p-3 border-t border-[rgba(10,20,32,0.10)] bg-[#F5F4F1]/95 space-y-2">
        {scanMsg && (
        ...
        <button
          ...
          className="w-full py-3 rounded-xl bg-white border border-[#0E9D98]/30 text-[#0E9D98] text-[13px] font-semibold flex items-center justify-center gap-2 disabled:opacity-60"
        >
        ...
        <button
          ...
          disabled={saving}
          className="w-full py-3 rounded-xl bg-[#0E9D98] hover:bg-[#0C8A85] text-white text-[13px] font-semibold tracking-wide flex items-center justify-center gap-2 disabled:opacity-60"
        >
          <Save size={14} />
