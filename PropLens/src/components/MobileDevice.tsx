...
export default function MobileDevice({ children }: MobileDeviceProps) {
  ...
  if (nativeMode) {
    return (
      <div className="relative flex flex-col w-full h-[100dvh] min-h-[100dvh] max-h-[100dvh] bg-[#F5F4F1] text-[#0A1420] font-sans select-none overflow-hidden">
        {/* A 40px strip used to sit here whenever the app was open in a browser
            rather than installed. It carried the wordmark and the words "Site
            mode" — the wordmark is repeated in the header directly below it, and
            "Site mode" is not a state the app can leave. On a 640px Android that
        ...
        <div className="h-[env(safe-area-inset-bottom,0px)] shrink-0 bg-[#F5F4F1]" />
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-900 p-4 md:p-8 select-none font-sans overflow-hidden">
      <div className="relative w-full max-w-[430px] aspect-[9/19.5] bg-neutral-950 rounded-[52px] p-4 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.9),0_0_0_1px_rgba(255,255,255,0.05),0_0_40px_10px_rgba(30,58,138,0.25)] border border-neutral-800 flex flex-col justify-stretch">
        ...
        <div className="absolute top-6.5 left-1/2 -translate-x-1/2 w-32 h-6.5 rounded-full z-50 pointer-events-none bg-[#0E9D98]/[0.04] opacity-30"></div>
        ...
        <div className="relative w-full h-full bg-[#F5F4F1] rounded-[40px] overflow-hidden flex flex-col border border-neutral-800 shadow-2xl">
          <div className="h-12 bg-[#F5F4F1] text-[#0A1420] px-7 flex items-center justify-between text-[13px] font-semibold tracking-wider z-40 shrink-0">
            <span className="text-[#0A1420]">{time}</span>
            <div className="flex items-center gap-2 text-[#0A1420]/70">
              <Signal size={12} className="text-[#0A1420]/50" />
              <span className="text-[13px] text-[#0A1420]/50 font-bold">5G</span>
              <Wifi size={12} className="text-indigo-500" />
              <Battery size={14} className="text-emerald-600 fill-emerald-500/20" />
            </div>
          </div>

          <div className="relative flex-1 w-full bg-[#F5F4F1] overflow-hidden flex flex-col">
            {children}
            <PwaInstallBanner />
          </div>

          <div className="h-6 bg-[#F5F4F1] flex items-center justify-center shrink-0 z-40">
            <div className="w-32 h-1 bg-[rgba(10,20,32,0.18)] rounded-full"></div>
          </div>
        </div>
      </div>
      ...
      <p className="mt-4 text-[13px] font-mono text-slate-500 flex items-center gap-2 max-w-md text-center">
        ...
        Desktop preview · On a phone, open this URL and install PropLens to Home Screen
      </p>
    </div>
  );
}
