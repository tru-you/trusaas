...
export default function CameraGuide({ property, onBack, onComplete, onPhotoCaptured, onEditRequested, onBulkPhotosUploaded }: CameraGuideProps) {
  ...
  // so onboarding a property reads as ~10 guided shots, not 27 fields.
  ...
  /* Draw a data-URI image onto the capture canvas and hand it to finalizeCapture,
     ...
     stayed on "Shoot" and the inspector had to press it on a photo they just picked. */
  ...
  /* Bulk "just these photos" dump — for a property where the inspector only has a
  ...
  const handleCapture = (e?: React.MouseEvent) => {
    ...
    // we NEVER fabricate an image. Send the inspector to the file picker instead.
    ...
    const report: QualityReport = {
      ...
      angleCheck: {
        ...
        feedback: angleStatus === 'Perfect'
          ? 'Frame angle is level and clear.'
          : angleStatus === 'Good'
          ? 'Satisfactory framing. Ensure the camera is level.'
    ...
    <div id="camera-guide-container" className="flex flex-col h-full bg-[#F5F4F1] text-[#0A1420] overflow-hidden relative">
      ...
      <div className="capture-preview relative flex-1 min-h-0 bg-black flex flex-col justify-center overflow-hidden">
        ...
        <div className="absolute top-0 inset-x-0 z-30 px-4 pt-3 pb-6 bg-gradient-to-b from-black/75 via-black/40 to-transparent pointer-events-none">
          ...
          {/* Bar tracks CORE progress — the goal is a listing-ready property, and the
              optional shots beyond core shouldn't make the bar look unfinished. */}
          <div className="mt-2 h-[3px] rounded-full bg-[rgba(10,20,32,0.10)] overflow-hidden">
            <div
      ...
      {/* Shot list — Core shots lead; the optional rest sit behind a toggle so
          ...
          past the fold. */}
      <div className="bg-[#EFEDE8] border-t border-[rgba(10,20,32,0.10)] py-2 shrink-0 z-10">
        ...
        <div className="px-3 pt-2">
          <button
            ...
            className="text-[12px] font-semibold text-[rgba(10,20,32,0.55)] hover:text-[#0A1420] flex items-center gap-1.5 cursor-pointer transition-colors"
          >
            {showMore
              ? 'Hide optional shots'
      ...
      <div className="bg-[#F5F4F1] p-4 border-t border-[rgba(10,20,32,0.10)] space-y-4 shrink-0">
        ...
        {activeSlot.id === 'service_book' && (
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-200 px-1">
            ...
            >
              <X size={12} className="stroke-[3]" /> No documents with property? Mark "None" (Exempt)
            </button>
          </div>
        )}
          ...
          <div className="flex flex-col gap-2">
            ...
            {onEditRequested && pendingShot.kind === 'photo' && (
              <button
                ...
                className="w-full py-2 rounded-xl text-[13px] text-[rgba(10,20,32,0.55)] hover:text-[#0A1420] border border-[rgba(10,20,32,0.10)] transition-colors"
              >
                Tag damage
              </button>
            )}
