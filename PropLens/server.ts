...
/** Constant-time compare that does not leak length via early return. Both
 * inputs are hashed first so the comparison always runs over 32-byte digests
 * regardless of the lengths of the raw codes — the early length check this
 * function used to do leaked the expected code's length via timing. */
...
Property details: ${propertyInfo?.address || 'address not provided'}${propertyInfo?.type ? `, ${propertyInfo.type}` : ''}${propertyInfo?.suburb ? `, ${propertyInfo.suburb}` : ''}.
