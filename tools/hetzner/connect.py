...
passwords = ["Paulie8891SAAS!", "Paulie8891SAAS", "Fruity22!@!@"]

for ip in ips:
    for pwd in passwords:
        try:
            print(f"Trying {ip} with password {pwd[:4]}***...")
            ssh = paramiko.SSHClient()
            ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            ssh.connect(ip, username="root", password=pwd, timeout=6)
            ...
            stdin, stdout, stderr = ssh.exec_command("uname -a && uptime && df -h /")
            print("Server status:\n" + stdout.read().decode())
            ssh.close()
            break
