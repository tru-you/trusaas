...
|---|---|---|---|
| **[TruChat](./truchat.md)** | 24/7 assistant — answers, stock search, lead capture, WhatsApp handoff | Yes (DMS-connected build) | Also a standalone WordPress build whose leads stay in its own portal. **Not on the widget CDN** — deployed per-site |
| **[TruAfford](./truafford.md)** | Affordability / soft pre-qual (budget → what they can spend) | Yes (Send + WhatsApp) | Estimate disclaimer in the result body |
...
| **TruBook** | Test-drive / showroom / trade-in booking | Yes (webhook + WhatsApp) | Modal, no launcher — opened via `TruDealer.open('book')` |
| **TruShare** | Per-vehicle social share sheet | No (pure links) | WhatsApp/FB/copy/native share; no backend |
| **TruValue** | Trade-in estimate against live market + shareable report | Yes (webhook + WhatsApp) | Direct-script embed only — **not loadable via the loader tag**; hits `POST /api/public/trade-estimate` on TruFlow |

All lead widgets capture via `data-webhook` (any Zapier/Make/CRM endpoint) **and/or** TruFlow's `/api/integration/webhook-lead` (when `data-slug` + `data-flow` are set), and can ping the dealer's WhatsApp via CallMeBot. The active source is `packages/standalone/` (deploys to `cdn.tru-saas.com`); the old `packages/tru-*` copies are frozen.
...
Or one **loader tag** that mounts several at once (the loader forwards `data-<widget>-<attr>` to each):

```html
<script src="https://cdn.tru-saas.com/tru-loader/tru-loader.js"
        data-dealer="True Cars" data-accent="#e30613" data-wa="27620502091"
        data-widgets="afford,repay,form,book,share"></script>
```
