...
1. **Start the inspection** against the vehicle (shares the same 27-item template as [TruLens](./trulens.md), so existing photos carry over).
...
3. **Let AI flag damage (optional).** Where enabled, detected issues surface as tagged points on the photos to prompt the inspector — the grade itself is set from what's entered plus tagged damage, not by the AI.
...
[SCREENSHOT: Inspection sheet — the 27-item checklist grouped by category, with status/condition controls and a recon-cost field on a damaged item]
...
- **What's built:** full 27-item inspection with status/condition/recon per item, an overall rating, AI-assisted damage detection on photos, market-value lookup with a manual cars.co.za / AutoTrader fallback, the trade-in value calculation, and two separately-referenced report documents (VIR and Trade-In). Installable as a PWA; records are retained per dealership.
- **Coming soon / partial:** the Basic (fully manual) vs. AI-assisted split is described as a product tier but is **not gated in the code** today — the AI damage-detection path and the manual path both run against the same inspection. Treat the tier packaging as a commercial decision, not an enforced feature flag.

## Related modules

- **[TruLens](./trulens.md)** — shares the identical 27-shot template; a car shot once serves both apps.
