 ...
 * Depends on: qualifier.js + options.config (or TRUPROPERTY_TRUCHAT_CONFIG / TRUPROPERTY_TRUCHAT_CONFIG).
 */
(function (root) {
  "use strict";
  ...
  function injectCss() {
    ...
    s.textContent = [
      ...
      "--tc-bg:#0f0f13;--tc-panel:rgba(18,18,20,.92);--tc-border:#2a2a30;--tc-text:#f8fafc;",
  ...
  function mount(container, options) {
    options = options || {};
    injectCss();
    var CFG = options.config || root.TRUPROPERTY_TRUCHAT_CONFIG || {};
    ...
    var pageVehicle = options.propertyInterest || CFG.pageVehicle || detectVehicleFromPage();
    ...
    if (pageVehicle) {
      bot.getSession().propertyInterest = pageVehicle;
    }
    ...
    container.innerHTML =
      ...
      '<div class="tc-composer"><input id="tcIn" type="text" placeholder="Ask about stock, finance, budget…" autocomplete="off" enterkeyhint="send" />' +
    ...
    function persistLead(force) {
      ...
      var hasSignal =
        ...
        (s.budgetNotes && s.budgetNotes !== "Awaiting input...") ||
        s.viewingDate ||
        s.inspectionDate ||
        (s.propertyInterest && s.propertyInterest !== "Browsing");
      if (!hasSignal) return;

      var sig = [s.name, s.phone, s.email, s.propertyInterest, s.qualified, s.viewingDate, s.inspectionDate].join("|");
      ...
      var entry = {
        ...
        phone: s.phone,
        email: s.email || "",
        propertyInterest: s.propertyInterest,
        budgetNotes: s.budgetNotes,
        viewingDate: s.viewingDate,
        inspectionDate: s.inspectionDate,
        bondInterest: s.bondInterest,
      ...
      if (s.qualified || force) {
        ...
        var notesBody = entry.ticket || ("TruChat lead — interest: " + (s.propertyInterest || "general"));
        if (transcript.length) {
          notesBody += "\n\n--- Chat transcript ---\n" + transcript.join("\n");
        }
        postWebhook(CFG.leadWebhook, {
          agency: CFG.agencySlug || CFG.dealerSlug || "",
          name: ((names[0] || "TruChat") + " " + (names.slice(1).join(" ") || "Lead")).trim(),
          contact: s.phone || "",
          email: s.email || "",
    ...
    function addCatalog(propertys) {
      var wrap = document.createElement("div");
      wrap.className = "tc-cars";
      (propertys || []).forEach(function (v) {
        ...
        card.querySelectorAll("button").forEach(function (btn) {
          btn.onclick = function () {
            ...
            if (useBackend) {
              userSay(a === "finance" ? "I'd like finance options on the " + label : "I'd like to viewing the " + label);
              return;
            }
    ...
    function addScheduler(scheduleType, days) {
      ...
      box.innerHTML =
        '<div class="tc-cal-t">' +
        (scheduleType === "test_drive" ? "Pick a viewing" : "Pick a valuation slot") +
    ...
    function handleOut(out) {
      (out.replies || []).forEach(function (r) {
        ...
        if (r.type === "catalog") addCatalog(r.propertys);
    ...
    function remoteTicket() {
      ...
      if (s.propertyInterest) msg += "⭐ *Interest:* " + s.propertyInterest + "\n";
      if (s.appointment) msg += "🗓️ *Booking:* " + s.appointment + "\n";
      if (s.budgetNotes) msg += "🔄 *Budget:* " + s.budgetNotes + "\n";
      if (s.bondInterest) msg += "💰 *Finance:* Interested\n";
    ...
    function handleBackendResult(res) {
      ...
      actions.forEach(function (a) {
        if (a.type === "stock" && a.propertys) addCatalog(a.propertys);
        if (a.type === "whatsapp") {
    ...
    function startChat() {
      ...
      var greet =
        ...
        "Hi — I'm **" +
          ...
          "**. Stock, viewings, budgets, hours — what do you need?";
      var sugs = CFG.suggestions || ["Browse stock", "Budget valuation", "Showroom hours", "Finance help"];
      if (pageVehicle) {
        greet =
          "Hi — I'm **" +
          (CFG.assistantName || "your assistant") +
          ...
          "**.\n\nYou're looking at **" +
          pageVehicle +
          "**. Want a viewing, finance options, or something else from the floor?";
        sugs = ["Test drive this car", "Finance help", "Browse other stock", "Budget"];
      }
      setTimeout(function () {
