...
(function () {
  ...
  var CSS = [
    ...
    "#" + ID + "-root .ta-panel{",
    "pointer-events:auto;",
    isInline ? "display:flex;flex-direction:column;width:100%;border-radius:18px;" : "display:none;flex-direction:column;width:min(390px,calc(100vw - 24px));",
    isInline ? "" : "max-height:min(78vh,640px);border-radius:20px;overflow:hidden;",
    ...
    isInline ? "" : "opacity:0;transform:translateY(16px) scale(.96);transition:opacity .35s var(--ta-ease),transform .4s var(--ta-spring)}",
    isInline ? "" : "#" + ID + "-root.is-open .ta-panel{display:flex;opacity:1;transform:none}",
    "#" + ID + "-root .ta-head{",
    ...
    isInline ? "" : "#" + ID + "-root .ta-panel{width:100%;max-height:min(82vh,680px)}}",
  ...
  /* ---------- host style inheritance ----------
     Reads the dealer site's computed styles (font, background luminance)
     and injects matching overrides into the shadow root so the widget
     feels native rather than foreign. */
  function inheritHostStyles() {
    try {
      var cs = getComputedStyle(document.body);
      var fontFamily = cs.fontFamily || "";
      var bgRaw = cs.backgroundColor || "rgb(255,255,255)";
      // Parse rgb(r,g,b)
      var m = bgRaw.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);
      var r = m ? +m[1] : 255, g = m ? +m[2] : 255, b = m ? +m[3] : 255;
      var luminance = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
      var isLightHost = luminance > 0.6;
      var extra = "";
      if (fontFamily) extra += "font-family:" + fontFamily + ";";
      if (isLightHost && cfg.theme !== "light") {
        // Host is light — override to light palette.
        cfg.theme = "light";
        extra += [
          "--ta-glass:rgba(251,248,243,.82);--ta-border:rgba(255,255,255,.55);",
          "--ta-text:#0E1A26;--ta-muted:rgba(14,26,38,.58);--ta-faint:rgba(14,26,38,.42);",
          "--ta-surface:rgba(251,248,243,.55);--ta-ink:#0E1A26;",
          "--ta-fill:rgba(14,26,38,.04);--ta-fill-2:rgba(14,26,38,.07);",
        ].join("");
      }
      return extra;
    } catch (e) { return ""; }
  ...
  function mount() {
    ...
    style.id = ID + "-css";
    style.textContent = CSS + inheritHostStyles();
    shadow.appendChild(style);

    var root = el(
      '<div id="' + ID + '-root" data-app="truafford" aria-live="polite"' + (isInline ? ' class="is-open"' : '') + '>' +
        '<div class="ta-panel" role="dialog" aria-label="Check affordability" aria-hidden="' + (isInline ? 'false' : 'true') + '">' +
          '<div class="ta-head">' +
