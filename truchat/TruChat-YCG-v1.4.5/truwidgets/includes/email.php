...
/** Plain-text dealer ticket for the WhatsApp alert. */
function truchat_dealer_ticket_text($lead) {
    $t = "🚗 NEW LEAD — " . truchat_opt('dealer_name', 'Your Car Guy') . "\n";
    if (!empty($lead['name']))            $t .= "👤 " . $lead['name'] . "\n";
    $t .= "📞 " . (!empty($lead['phone']) ? $lead['phone'] : 'no number') . "\n";
    if (!empty($lead['email']))           $t .= "✉️ " . $lead['email'] . "\n";
    if (!empty($lead['vehicleInterest'])) $t .= "⭐ " . $lead['vehicleInterest'] . "\n";
    if (!empty($lead['appointment']))     $t .= "🗓️ " . $lead['appointment'] . "\n";
    if (!empty($lead['tradeInDetails']))  $t .= "🔄 " . $lead['tradeInDetails'] . "\n";
    if (!empty($lead['financeInterest'])) $t .= "💰 Finance interest\n";
    return trim($t);
}

/** Send a WhatsApp alert to the dealer's own number via CallMeBot (free relay). */
function truchat_send_whatsapp_alert($lead) {
    $phone  = preg_replace('/[^\d]/', '', truchat_opt('callmebot_phone', ''));
    $apikey = truchat_opt('callmebot_apikey', '');
    if ($phone === '' || $apikey === '') return false;
    $url = 'https://api.callmebot.com/whatsapp.php?phone=' . rawurlencode($phone)
        . '&text=' . rawurlencode(truchat_dealer_ticket_text($lead))
        . '&apikey=' . rawurlencode($apikey);
    $res = wp_remote_get($url, array('timeout' => 15));
    if (is_wp_error($res)) { error_log('[TruChat] WhatsApp alert failed: ' . $res->get_error_message()); return false; }
    return wp_remote_retrieve_response_code($res) < 400;
}

/**
 * Notify on a new lead. Dealer alert goes by the chosen channel(s); the customer
 * always gets the email confirmation. Returns a status report.
 */
function truchat_send_lead_emails($session, $summary) {
    $out = array('dealer' => false, 'customer' => false, 'wa' => false, 'skipped' => false);
    $lead = array_merge($session, array('summary' => $summary));
    $headers = truchat_mail_headers();
    $channel = truchat_opt('dealer_alert_channel', 'email'); // email | whatsapp | both
    ...
    // Dealer alert — WhatsApp (CallMeBot, free relay to the dealer's own number)
    if ($channel === 'whatsapp' || $channel === 'both') {
        $out['wa'] = truchat_send_whatsapp_alert($lead);
    }

    // Customer confirmation — always email (WhatsApp to a customer needs the Cloud API)
    ...
    if (!$out['dealer'] && !$out['customer'] && !$out['wa']) $out['skipped'] = true;
    return $out;
}
