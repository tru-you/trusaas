...
export default function App() {
  ...
  const AUTH_SESSION_KEY = "truflow_lite_session";
  ...
  // Filter state for non-admins
  const filteredVehicles = state ? (isMasterAdmin ? state.vehicles : state.vehicles.filter(v => v.dealershipId === dealershipId)) : [];
  ...
  const filteredNavigation = groupedNavigation.map(group => {
    ...
    if (selectedRole === 'salesperson') {
      items = items.filter(item => 
        ['dashboard', 'inventory', 'upload', 'leads', 'tasks', 'accounting_recon'].includes(item.id)
      );
  ...
  const handlePublishVehicle = async (e: React.FormEvent) => {
    ...
    addNotification(
      "Vehicle on floor",
      `${newVehicleForm.year} ${newVehicleForm.make} ${newVehicleForm.model} (${stock}) — shoot in TruLens, then Export to DMS`,
      "info"
    );
    ...
    !isLoggedIn ? (
      ...
      >
        <div className="mb-6 flex flex-col items-center">
          ...
          <a href="https://www.trusaas.co.za" target="_blank" rel="noopener noreferrer" className="font-mono text-[9px] text-[#15C7C0] hover:underline tracking-widest mt-2 uppercase">www.trusaas.co.za</a>
      ...
      <main className="flex-1 md:ml-[240px] min-h-screen px-4 py-6 md:px-8 md:py-8 z-10 flex flex-col gap-6 max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col gap-2 md:hidden border-b border-white/5 pb-3">
          <div className="flex justify-between items-center">
            ...
            <img src={logo} alt="TruFlow Lite" className="h-9 w-auto max-w-[160px] object-contain logo-float" />
            <button
              type="button"
              onClick={handleLogout}
