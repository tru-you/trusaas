...
/** Load from TruFlow Lite server so TruLens photo exports are visible. */
export async function fetchState(): Promise<DMSState> {
  try {
    ...
    if (res.ok) {
      ...
      try {
        localStorage.setItem("dms_state_lite", JSON.stringify(data));
      } catch {
        /* ignore large photo quota */
      }
      return data;
    }
  } catch (err) {
    console.warn("Lite fetchState server failed, using cache", err);
  }

  const local = localStorage.getItem("dms_state_lite") || localStorage.getItem("dms_state");
  if (local) {
    try {
  ...
  localStorage.setItem("dms_state_lite", JSON.stringify(DEFAULT_MOCK_STATE));
...
async function updateState(mutator: (state: DMSState) => void): Promise<DMSState> {
  ...
  mutator(state);
  try {
    localStorage.setItem("dms_state_lite", JSON.stringify(state));
  } catch {
    /* ignore */
  }
  return state;
...
export async function resetState(): Promise<DMSState> {
  try {
    ...
    if (res.ok) {
      ...
      localStorage.setItem("dms_state_lite", JSON.stringify(next));
      return next;
    }
  } catch {
    /* fall through */
...
export async function refreshFromServer(): Promise<DMSState> {
  const res = await fetch("/api/state", { cache: "no-store" });
  if (!res.ok) throw new Error(`Lite DMS refresh failed (${res.status})`);
  const data = (await res.json()) as DMSState;
  try {
    localStorage.setItem("dms_state_lite", JSON.stringify(data));
  } catch {
