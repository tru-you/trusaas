...
const app = express();
// Port 3002 so it does not clash with TruLens (3000) or TruFlow Premium (3001)
const PORT = Number(process.env.PORT) || 3002;

// Large payloads for TruLens photo export
...
// --- PUBLIC STOCK FEED (dealer HTML websites + embed widget) ---

function toPublicVehicle(v: any, source: string = "lite") {
  const images = Array.isArray(v.images) ? v.images.filter(Boolean) : [];
  const extras = Array.isArray((v as any).extrasPhotos) ? (v as any).extrasPhotos.filter(Boolean) : [];
  const allImages = [...images, ...extras];
  const published = (v as any).showOnWebsite !== false && v.status === "INVENTORY";
  ...
  return {
    ...
    model: v.model,
    trim: v.trim || "",
    price: v.retailPrice ?? (v as any).price ?? 0,
    ...
    truPrice: (v as any).truPrice ? Number((v as any).truPrice) : undefined,
    mileage: v.mileage ?? 0,
    ...
    source: (v as any).source || source,
    updatedAt: (v as any).lastPhotoSync || null,
  };
}
...
app.get("/api/public/stock", (req, res) => {
  ...
  res.json(buildPublicStock(readState(), dealer));
});
...
// --- TRULENS / AUTOLENS PHOTO PUSH (same contract as TruFlow Premium) ---
...
// Push photos FROM TruLens INTO TruFlow Lite (create vehicle if missing)
app.post("/api/sync/push-photos", (req, res) => {
  try {
    ...
    const matchStock = stockNumber || vehicleMeta.stockNumber || null;
    ...
    if (idx === -1) {
      if (!createIfMissing) {
        return res.json({
          synced: false,
          message: "Vehicle not found in TruFlow Lite. Create it first or set createIfMissing=true.",
      ...
      const newVehicle = {
        ...
        stockNumber: matchStock || "STK-" + Math.floor(Math.random() * 900000 + 100000),
        dateAcquired: now,
        daysInInventory: 1,
        description:
      ...
      state.vehicles.unshift(newVehicle as any);
      writeState(state);

      return res.status(201).json({
        synced: true,
        created: true,
        message: `Created TruFlow Lite vehicle and pushed ${photoEntries.length} photos from TruLens.`,
        breakdown: {
    ...
    (state.vehicles[idx] as any).damagePhotos = mapped.damagePhotos;
    (state.vehicles[idx] as any).vinPhotos = mapped.vinPhotos;
    (state.vehicles[idx] as any).serviceBookPhotos = mapped.serviceBookPhotos;
    (state.vehicles[idx] as any).extrasPhotos = mapped.extrasPhotos;
    (state.vehicles[idx] as any).lastPhotoSync = new Date().toISOString();
    (state.vehicles[idx] as any).source = "trulens";
    ...
    res.json({
      synced: true,
      created: false,
      message: `Pushed ${photoEntries.length} photos from TruLens to TruFlow Lite vehicle ${state.vehicles[idx].stockNumber}.`,
      breakdown: {
        mainImages: mapped.images.length,
        extras: mapped.extrasPhotos.length,
        damage: mapped.damagePhotos.length,
    ...
    console.error("Lite push photo sync error:", error);
    res.status(500).json({
      synced: false,
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`TruFlow Lite active on http://localhost:${PORT} (TruLens export: POST /api/sync/push-photos)`);
  });
}
