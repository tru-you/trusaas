...
// motion removed to avoid duplicate React from framer-motion v12
...
export default function CameraGuide({ vehicle, onBack, onPhotoCaptured, onBulkPhotosUploaded }: CameraGuideProps) {
  ...
  const photos: Record<string, string> = (vehicle as any).photos || {};
  ...
  React.useEffect(() => {
    async function startCamera() {
      try {
        ...
        console.warn('Camera access not granted or not available. Running high-fidelity car pose simulator.');
  ...
  const handleCapture = () => {
    ...
    // Draw frame (either camera, custom uploaded file, or simulated graphics)
    if (isCameraActive && videoRef.current) {
      // Crop & Draw video feed
      ctx.drawImage(videoRef.current, 0, 0, canvas.width, canvas.height);
    } else if (customFile) {
      // Draw uploaded picture
      const img = new Image();
      img.onload = () => {
      ...
      // Render the highly sophisticated simulated car scene
      drawSimulatedCarScene(ctx, canvas.width, canvas.height);
    }
    ...
    <div id="camera-guide-container" className="flex flex-col h-full bg-neutral-950 text-white overflow-hidden relative">
      ...
      <div className="relative flex-1 bg-black flex flex-col justify-center overflow-hidden">
        ...
        {isCameraActive ? (
          ...
          /* Empty Viewport Placeholder when no camera/file */
          <div className="w-full h-full bg-neutral-900 flex flex-col items-center justify-center p-8 text-center space-y-3">
            <div className="p-4 rounded-full bg-neutral-800 text-neutral-600">
              <Camera size={48} strokeWidth={1.5} />
            </div>
            <div className="space-y-1">
              <p className="text-sm font-bold text-neutral-300">Camera Feed Inactive</p>
              <p className="text-[10px] text-neutral-500 max-w-[200px]">
                Please allow camera permissions or upload a photo to use the alignment guides.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-1.5 overflow-x-auto pb-1 px-1.5 scrollbar-none">
          {phaseSlots.map(slot => {
            ...
            return (
              ...
              >
                {isTaken && <Check size={10} className="text-emerald-400 font-extrabold" />}
      ...
      <div className="bg-neutral-950 p-4 border-t border-neutral-850 space-y-4 shrink-0">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          {/* File Upload Trigger */}
          <label className="flex-1 py-2 bg-neutral-900 border border-neutral-800 hover:bg-neutral-850 rounded-xl text-[9px] font-bold text-neutral-300 flex flex-col items-center justify-center gap-1 cursor-pointer text-center">
            <Upload size={13} className="text-neutral-400" /> Upload {activeSlot.id === 'video_360' ? 'Video' : 'Photo'}
            <input 
              type="file" 
          ...
          >
            <Smartphone size={13} /> Level Guide
          </button>
        </div>
