    ...
    <div className="fixed bottom-5 left-5 z-[1000] font-sans">
      ...
      {isOpen && (
        <div className="flex flex-col w-[350px] h-[480px] bg-[color:var(--ink-2)] border border-[rgba(126,164,214,0.15)] rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-5 duration-200">
          ...
          <div className="flex justify-between items-center px-4 py-3 border-b border-[rgba(126,164,214,0.1)] bg-[linear-gradient(90deg,rgba(21,199,192,0.1),rgba(20,102,224,0.05))]">
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col">
                <span className="font-bold text-[16px] text-[color:var(--white)]">Website Chat Bot</span>
                <span className="text-[13px] text-[rgba(232,234,230,0.72)]">Public Customer Assistant</span>
              </div>
            </div>
            <button
              onClick={() => setIsOpen(false)}
              className="text-[rgba(232,234,230,0.72)] hover:text-[color:var(--white)] transition-colors p-1 rounded-lg hover:bg-white/5 cursor-pointer"
            >
              <X size={16} />
            </button>
          </div>
          ...
          {/* Messages Container */}
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3 min-h-0 scrollbar-thin bg-[color:var(--ink)]">
            ...
            {showLeadForm && (
              <div className="self-start bg-[color:var(--ink-2)] border border-[color:var(--cyan-soft)] rounded-xl rounded-bl-none shadow-sm p-3 w-[90%] text-[13px] animate-in slide-in-from-left-2 text-[color:var(--white)]">
                <p className="font-bold text-[color:var(--white)] mb-2">Can we get your details?</p>
                <form onSubmit={handleLeadSubmit} className="flex flex-col gap-2">
                  <input required placeholder="Name" value={leadForm.name} onChange={(e) => setLeadForm({...leadForm, name: e.target.value})} className="bg-[color:var(--ink-2)] border border-white/10 rounded px-2 py-2 text-[color:var(--white)] placeholder-[rgba(232,234,230,0.45)] outline-none focus:border-[color:var(--cyan)]" />
                  <input required type="tel" placeholder="Phone Number" value={leadForm.phone} onChange={(e) => setLeadForm({...leadForm, phone: e.target.value})} className="bg-[color:var(--ink-2)] border border-white/10 rounded px-2 py-2 text-[color:var(--white)] placeholder-[rgba(232,234,230,0.45)] outline-none focus:border-[color:var(--cyan)]" />
                  <input type="email" placeholder="Email (Optional)" value={leadForm.email} onChange={(e) => setLeadForm({...leadForm, email: e.target.value})} className="bg-[color:var(--ink-2)] border border-white/10 rounded px-2 py-2 text-[color:var(--white)] placeholder-[rgba(232,234,230,0.45)] outline-none focus:border-[color:var(--cyan)]" />
                  <button type="submit" className="bg-[color:var(--cyan)] on-fill font-bold py-2 rounded mt-1 cursor-pointer hover:bg-[color:var(--cyan-soft)] transition-colors">Send to Dealer</button>
                </form>
              </div>
            )}
          ...
          >
            <input
              ...
              className="flex-1 bg-[color:var(--ink)] border border-white/10 rounded-full px-4 py-2 text-[13px] text-[color:var(--white)] placeholder-[rgba(232,234,230,0.45)] outline-none focus:border-[color:var(--cyan)] focus:bg-[color:var(--cyan-faint)] transition-all"
            />
      ...
      {!isOpen && (
        <div className="relative group cursor-pointer animate-none" onClick={() => setIsOpen(true)}>
          <div className="absolute -top-10 left-0 bg-[color:var(--ink-2)] shadow-lg text-[color:var(--white)] text-[13px] font-bold px-3 py-2 rounded-lg border border-white/5 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-[1001]">
            Chat with us live!
