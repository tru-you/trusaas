...
/** Production defaults (*.trudealers.com, *.tru-saas.com, *.onrender.com); localhost when developing on PC */
function isTruSaasHost(): boolean {
  try {
    if (typeof window === "undefined") return false;
    const h = window.location.hostname;
    return /\.onrender\.com$/i.test(h) || /(^|\.)tru-saas\.com$/i.test(h) || /\.?trusaas\.co\.za$/i.test(h) || /(^|\.)trudealers\.com$/i.test(h);
  } catch {
    return false;
  }
}
