...
(function (root) {
  ...
  function createQualifier(config) {
    ...
    function computeDigitalScore() {
      ...
      if (session.vehicleInterest && session.vehicleInterest !== "Browsing") s += 15;
      if (session.financeInterest) s += 15;
      if (session.tradeInDetails && session.tradeInDetails !== "Awaiting input...") s += 15;
      if (session.testDriveDate) s += 20;
    ...
    function process(text) {
      ...
      if (emailMatch) {
        ...
        say(
          ...
          session.phone ? ["Browse stock", "Trade-in", "Hours"] : []
        );
      ...
      // Name — "my name is"/"call me" capture up to the next clause break, and
      // "i'm"/"i am" only count when the WHOLE remaining message is just the
      // name (1-2 words). Without that second guard, "I'm interested in a
      // Hilux" matched "i'm" and swallowed the next 20 chars as a name —
      // "interested in a Hilu" — because the old pattern had no way to tell
      // a name introduction from any other "I'm ..." sentence.
      const nameMatched =
        raw.match(/(?:my name is|call me)\s+([a-zA-Z][a-zA-Z\s]{1,24}?)(?:[,.!]|$)/i) ||
        raw.match(/^i(?:'m| am)\s+([a-zA-Z]+(?:\s[a-zA-Z]+)?)\s*[,.!]?\s*$/i);
      if (nameMatched) {
      ...
      if (/^\s*(hi+|hey+|hello|howzit|hallo|yo|greetings|good\s+(morning|afternoon|evening|day))[\s!.,]*$/i.test(raw)) {
        say(
          "Hi there! 👋 I'm **" +
            ...
            "** at **" +
            cfg.dealerName +
            "**. I can help with **stock**, **test drives**, **trade-in valuations**, **finance**, or **affordability**. What are you after?",
          ["Browse stock", "Calculate Finance", "Check Affordability", "Book Appointment"]
        );
        return { replies: replies, session: session };
      }
    ...
    function selectVehicleAction(vehicleLabel, action) {
      ...
      if (action === "finance") {
        ...
        addScore(15, "finance on vehicle");
        replies.push({
          type: "text",
          text:
        ...
        addScore(15, "test drive request");
        replies.push({
          type: "text",
    ...
    function getIdleNudge() {
      ...
      if (session.vehicleInterest && session.vehicleInterest !== "Browsing") {
        return {
          text: "Still thinking about the **" + session.vehicleInterest + "**? I can check finance options or book a test drive — just say the word.",
          suggestions: ["Finance help", "Book test drive", "Browse other stock"]
        };
      }
      if (session.financeInterest) {
        return {
          text: "Still here if you need finance help — share your **phone number** and the finance desk can start a soft assessment.",
          suggestions: ["Browse stock", "Trade-in"]
        };
      }
      return {
        text: "Still around! Anything I can help with — **stock**, **test drives**, **trade-ins**, or **finance**?",
        suggestions: ["Browse stock", "Trade-in valuation", "Finance help"]
      };
    }
