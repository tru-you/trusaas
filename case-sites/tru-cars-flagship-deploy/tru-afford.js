...
(function () {
  ...
  var CSS = [
    ...
    "@media (max-width:640px){#" + ID + "-root{right:auto;left:12px;align-items:flex-start;max-width:none}",
    "#" + ID + "-root .ta-launcher{max-width:52px;width:52px;height:52px;border-radius:50%;padding:0;justify-content:center;overflow:hidden}",
    "#" + ID + "-root .ta-launcher span:not(.ta-ico){display:none}",
    "#" + ID + "-root .ta-launcher .ta-ico{margin:0}",
    "#" + ID + "-root.is-open{right:12px;left:12px}",
  ...
  function render() {
    ...
    if (state.step === 1) {
      ...
      html =
        ...
        '<button type="button" class="ta-btn ta-btn-primary" id="ta-submit">Submit to Dealer</button>' +
        "</div>" +
        (cfg.wa ? '<div class="ta-actions" style="margin-top:8px"><button type="button" class="ta-btn ta-btn-wa" id="ta-wa" style="flex:1">WhatsApp Result</button></div>' : '') +
    ...
    if (submitBtn) {
      submitBtn.addEventListener("click", function () {
        ...
        if (!state.phone || state.phone.replace(/\D/g, "").length < 7) {
          var ph = $("#ta-phone");
          if (ph) { ph.style.borderColor = "rgba(239,68,68,.6)"; ph.focus(); }
          return;
        }
        state.result = calc();
        var r = state.result;
        var msg = [
          "TruAfford soft estimate for " + cfg.dealer,
          "Name: " + (state.name || "—"),
          "Phone: " + (state.phone || "—"),
          ...
          try {
            fetch(cfg.flowUrl.replace(/\/$/, "") + "/api/integration/webhook-lead", {
              ...
              body: JSON.stringify({
                ...
                phone: state.phone || "",
                email: "",
                source: "TruAfford Widget (WhatsApp)",
                notes: msg
              }),
              mode: "cors",
              keepalive: true
