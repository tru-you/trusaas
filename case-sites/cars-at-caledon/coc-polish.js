...
function tradeTool(){
  ...
  root.querySelector("#ctBook").addEventListener("click", function(){
    ...
    var msg="Hi Cars on Caledon, I'd like a LIVE TRADE OFFER on my car via TruLive.\n"
      +"Car: "+car+"\n"
      +(km?"Odometer: "+km+"\n":"")
      +(cond?"Condition: "+cond+"\n":"")
...
function navStatus(){
  ...
  var parts, day, mins;
  try{
    parts = new Intl.DateTimeFormat("en-ZA",{timeZone:"Africa/Johannesburg",weekday:"short",hour:"2-digit",minute:"2-digit",hour12:false}).formatToParts(new Date());
  ...
  if(hb) hb.textContent = open
    ? "Open now · 257 Caledon Street, Kariega"
