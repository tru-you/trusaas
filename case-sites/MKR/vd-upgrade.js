/* MKR Vehicle Detail Upgrade — Tru3D, VIR, TruLens AI, TruPrice, Booking */
(function(){
...
window.openVehicleDetail = function(car){
  var _fmtR = window.fmtR || MKR.fmtR || function(n){return "R "+Number(n||0).toLocaleString("en-ZA");};
  var _monthly = window.monthly || MKR.monthly || function(p,d,t){d=d||.1;t=t||72;var r=.1175/12;return Math.round(p*(1-d)*r/(1-Math.pow(1+r,-t)));};
  var _WA = window.WA || "27662912809";

  var wa = encodeURIComponent("Hi MKR, I'm interested in the "+car.y+" "+car.make+" "+car.name+" ("+_fmtR(car.price)+") - "+car.tag+". Is it still available?");
  var waReserve = encodeURIComponent("Hi MKR, I'd like to reserve the "+car.y+" "+car.make+" "+car.name+" ("+_fmtR(car.price)+") with the R2 500 refundable deposit");
  var waFinance = encodeURIComponent("Hi MKR, I'd like to apply for finance on the "+car.y+" "+car.make+" "+car.name+" ("+_fmtR(car.price)+")");
  ...
  var bodyName = {suv:"SUV",bakkie:"Bakkie",hatch:"Hatchback",sedan:"Sedan",coupe:"Coupe"}[car.body]||car.body;
  ...
  var tpDelta = (typeof MKR !== "undefined" && MKR.priceDelta) ? MKR.priceDelta(car) : { below: false, amount: 0, pct: 0 };
  ...
  vdContent.innerHTML =
    ...
    +'<div class="vd-vir-eyebrow" style="display:flex;align-items:center;gap:8px"><img src="mkr-badge.jpg" alt="" style="width:22px;height:22px;border-radius:6px;object-fit:cover">Verified Inspection Report</div>'
    ...
    +'<div class="vd-buy">'
    +'<div class="vd-crumb">MKR Auto Sales / '+(car.tag||"Featured")+' / '+car.make+'</div>'
    ...
    +'<span>Reserve this car &middot; R2 500 refundable</span></a>'
    +'<div class="row2">'
    +'<a class="btn btn-dark" href="https://wa.me/'+_WA+'?text='+encodeURIComponent("Hi MKR, I'd like a test drive of the "+car.y+" "+car.make+" "+car.name)+'" target="_blank" rel="noopener">'
    ...
    +'<span><i style="background:var(--gold)"></i>AA Authorised</span>'
    +'</div></div>'
    ...
    +'<a class="mb-wa" href="https://wa.me/'+_WA+'?text='+wa+'" target="_blank" rel="noopener">WhatsApp</a>'
    +'</div>';
  ...
  // share this car — copy WhatsApp-ready message
  var shareBtn = document.getElementById("vdShareBtn");
  if(shareBtn){
    shareBtn.addEventListener("click",function(){
      var shareText = "Check out this "+car.y+" "+car.make+" "+car.name+" at MKR Auto Sales — "+_fmtR(car.price)+" (from "+_fmtR(pm)+"/pm). VIR-inspected, nationwide delivery. https://wa.me/"+_WA;
      function done(){
        var toast = document.createElement("div");
        toast.className = "vd-share-toast";
        toast.textContent = "Copied — paste it to anyone on WhatsApp";
...
window.openBookingModal = function(car){
  ...
  bg.innerHTML = '<div class="vd-booking">'
    +'<div class="vd-booking-head"><div style="display:flex;align-items:center;gap:10px"><img src="mkr-badge.jpg" alt="MKR" style="width:34px;height:34px;border-radius:9px;object-fit:cover;flex-shrink:0"><h3>Book a live video viewing</h3></div><span class="vd-booking-close" id="vdBookingClose">&times;</span></div>'
    ...
    +'<div class="vd-booking-note">You\'ll be connected to our showroom team on WhatsApp to confirm the viewing. No charge, no obligation.</div>'
    +'</div>';
  ...
  bg.querySelector("#vdBookConfirm").addEventListener("click",function(){
    ...
    var msg = encodeURIComponent("Hi MKR, I'd like to book a live video viewing of the "+car.y+" "+car.make+" "+car.name+" ("+_fmtR(car.price)+") on "+dayTxt+" at "+timeTxt+". Please confirm.");
