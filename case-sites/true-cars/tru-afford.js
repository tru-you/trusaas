...
(function () {
  ...
  /* True-Cars brand: signal blue → teal (matches .btn-grad) */
  var CSS = [
    "#" + ID + "-root{",
    "--ta-signal:#1466E0;--ta-signal-bright:#4D9BFF;--ta-signal-deep:#0B49B0;",
    "--ta-blue:#1466E0;--ta-blue-bright:#4D9BFF;--ta-blue-deep:#0B49B0;",
    "--ta-teal:#15C7C0;--ta-teal-bright:#4FE3DC;",
    "--ta-grad:linear-gradient(120deg,#1466E0 0%,#1E86D2 50%,#15C7C0 100%);",
    ...
    "display:flex;flex-direction:column;align-items:flex-end;gap:8px;max-width:min(340px,calc(100vw - 24px));pointer-events:none}",
    ...
    "border:1px solid rgba(20,102,224,.28);color:#fff;",
    "box-shadow:0 12px 36px -10px rgba(0,0,0,.55),0 0 24px -8px rgba(20,102,224,.35),inset 0 1px 0 rgba(255,255,255,.1);",
    "backdrop-filter:blur(18px) saturate(1.4);-webkit-backdrop-filter:blur(18px) saturate(1.4);",
    "transition:transform .35s var(--ta-spring),box-shadow .35s var(--ta-ease);max-width:194px;text-align:left}",
    "#" + ID + "-root .ta-launcher:hover{transform:translateY(-3px) scale(1.02);",
    "box-shadow:0 18px 40px -10px rgba(20,102,224,.4),0 0 32px -6px rgba(59,130,246,.3),inset 0 1px 0 rgba(255,255,255,.14)}",
    "#" + ID + "-root .ta-launcher:active{transform:scale(.97)}",
    "#" + ID + "-root .ta-ico{",
    "width:32px;height:32px;border-radius:50%;flex-shrink:0;",
    "background:var(--ta-grad);",
    "display:grid;place-items:center;box-shadow:0 6px 18px -4px rgba(20,102,224,.55);position:relative}",
    "#" + ID + "-root .ta-ico::after{content:'';position:absolute;inset:-4px;border-radius:50%;",
    "border:2px solid rgba(20,102,224,.45);animation:taPulse 2.2s ease-out infinite}",
    "@keyframes taPulse{0%{transform:scale(1);opacity:.6}100%{transform:scale(1.45);opacity:0}}",
    "#" + ID + "-root .ta-ico svg{width:16px;height:16px}",
    "#" + ID + "-root .ta-ltitle{display:block;font-size:11.5px;font-weight:700;line-height:1.2;letter-spacing:.01em}",
    "#" + ID + "-root .ta-lsub{display:block;font-size:9px;color:#a8b0c0;line-height:1.3;margin-top:1px;max-width:130px}",
    "#" + ID + "-root .ta-badge{font-size:7.5px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;",
    "color:var(--ta-signal-bright);margin-top:2px;display:block}",
    "#" + ID + "-root.is-open .ta-launcher{display:none}",
    "#" + ID + "-root .ta-panel{",
    "pointer-events:auto;display:none;flex-direction:column;width:min(324px,calc(100vw - 24px));",
    "max-height:min(72vh,540px);border-radius:18px;overflow:hidden;",
    ...
    "box-shadow:0 28px 70px -20px rgba(0,0,0,.65),0 0 40px -16px rgba(20,102,224,.2),inset 0 1px 0 rgba(255,255,255,.1);",
    ...
    "#" + ID + "-root .ta-head{",
    "padding:11px 12px 10px;display:flex;align-items:center;gap:9px;",
    "background:var(--ta-grad);",
    "border-bottom:1px solid rgba(255,255,255,.1);flex-shrink:0}",
    "#" + ID + "-root .ta-head b{display:block;font-size:12.5px;font-weight:800}",
    "#" + ID + "-root .ta-head span{font-size:10px;opacity:.9}",
    "#" + ID + "-root .ta-x{",
    "margin-left:auto;width:28px;height:28px;border-radius:9px;background:rgba(255,255,255,.16);",
    "color:#fff;font-size:16px;display:grid;place-items:center;transition:background .2s,transform .25s var(--ta-spring)}",
    "#" + ID + "-root .ta-x:hover{background:rgba(255,255,255,.26);transform:scale(1.05)}",
    "#" + ID + "-root .ta-steps{display:flex;gap:4px;padding:9px 12px 0;flex-shrink:0}",
    ...
    "#" + ID + "-root .ta-body{padding:11px 12px 12px;overflow-y:auto;flex:1;min-height:0}",
    ...
    "#" + ID + "-root .ta-h{font-size:15px;font-weight:800;letter-spacing:-.02em;margin:0 0 5px;line-height:1.25}",
    "#" + ID + "-root .ta-p{font-size:11.5px;color:var(--ta-muted);line-height:1.5;margin:0 0 12px}",
    "#" + ID + "-root .ta-soft{",
    "display:flex;align-items:flex-start;gap:7px;padding:9px 10px;border-radius:11px;margin-bottom:12px;",
    "background:rgba(20,184,166,.1);border:1px solid rgba(20,184,166,.3);font-size:10.5px;color:#99f6e4;line-height:1.45}",
    "#" + ID + "-root .ta-soft svg{width:14px;height:14px;flex-shrink:0;margin-top:1px;stroke:var(--ta-teal-bright)}",
    "#" + ID + "-root label{display:block;font-size:9px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:var(--ta-muted);margin:0 0 5px}",
    "#" + ID + "-root .ta-field{margin-bottom:12px}",
    "#" + ID + "-root .ta-row{display:flex;justify-content:space-between;align-items:center;margin-bottom:6px}",
    "#" + ID + "-root .ta-row output{font-weight:800;font-size:12px;color:#fff}",
    ...
    "background:#fff;border:4px solid var(--ta-signal);box-shadow:0 2px 10px rgba(20,102,224,.45);cursor:grab}",
    "#" + ID + "-root input[type=text],#" + ID + "-root input[type=tel],#" + ID + "-root select{",
    "width:100%;padding:10px 11px;border-radius:11px;border:1.5px solid rgba(255,255,255,.12);",
    "background:rgba(255,255,255,.06);color:#fff;font:600 12.5px Inter,system-ui,sans-serif;outline:none;",
    "transition:border-color .2s,box-shadow .2s}",
    "#" + ID + "-root input:focus,#" + ID + "-root select:focus{",
    "border-color:rgba(20,102,224,.55);box-shadow:0 0 0 3px rgba(20,102,224,.22)}",
    ...
    "#" + ID + "-root .ta-pill{",
    "padding:8px 11px;border-radius:100px;font-size:10.5px;font-weight:700;",
    ...
    "box-shadow:0 8px 18px -8px rgba(20,102,224,.5)}",
    "#" + ID + "-root .ta-actions{display:flex;gap:8px;margin-top:6px}",
    "#" + ID + "-root .ta-btn{",
    "flex:1;display:inline-flex;align-items:center;justify-content:center;gap:8px;",
    "padding:10px 12px;border-radius:11px;font-weight:800;font-size:12px;",
    ...
    "box-shadow:0 10px 26px -10px rgba(20,102,224,.55),inset 0 1px 0 rgba(255,255,255,.2)}",
    "#" + ID + "-root .ta-btn-primary:hover{filter:brightness(1.08);transform:translateY(-2px);",
    "box-shadow:0 14px 32px -10px rgba(20,102,224,.55),0 0 24px -8px rgba(59,130,246,.35)}",
    ...
    "background:linear-gradient(160deg,rgba(20,102,224,.2),rgba(37,99,235,.12),rgba(255,255,255,.03));",
    "border:1px solid rgba(20,102,224,.25)}",
    ...
    "#" + ID + "-root .ta-big{font-size:22px;font-weight:900;letter-spacing:-.03em;line-height:1.1;margin:4px 0 2px;",
    "background:linear-gradient(120deg,#4D9BFF,#4FE3DC 55%,#7FE9E3);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}",
    "#" + ID + "-root .ta-range{font-size:11px;color:var(--ta-muted);margin-bottom:11px}",
    "#" + ID + "-root .ta-metrics{display:grid;grid-template-columns:1fr 1fr;gap:7px}",
    "#" + ID + "-root .ta-metric{padding:9px;border-radius:11px;background:rgba(0,0,0,.25);border:1px solid rgba(255,255,255,.06)}",
    "#" + ID + "-root .ta-metric .k{font-size:8.5px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:var(--ta-muted)}",
    "#" + ID + "-root .ta-metric .v{font-size:12.5px;font-weight:800;margin-top:3px}",
    "#" + ID + "-root .ta-fine{font-size:9.5px;color:rgba(148,163,184,.75);line-height:1.45;margin-top:11px}",
    "#" + ID + "-root .ta-foot{padding:9px 12px 10px;border-top:1px solid rgba(255,255,255,.08);flex-shrink:0;",
    "font-size:9.5px;color:var(--ta-muted);text-align:center}",
    "#" + ID + "-root .ta-foot b{color:var(--ta-signal-bright)}",
    /* ── mobile: compact pill on the left, clear of the back-to-top button ── */
    "@media (max-width:480px){",
    "#" + ID + "-root{left:14px;right:auto;bottom:calc(132px + env(safe-area-inset-bottom,0px));",
    "align-items:flex-start;max-width:calc(100vw - 28px)}",
    "#" + ID + "-root .ta-launcher{max-width:none;width:auto;padding:6px 12px 6px 6px;gap:7px}",
    "#" + ID + "-root .ta-lsub,#" + ID + "-root .ta-badge{display:none}",
    "#" + ID + "-root .ta-ltitle{font-size:11px;white-space:nowrap}",
    "#" + ID + "-root .ta-ico{width:28px;height:28px}",
    "#" + ID + "-root .ta-ico svg{width:15px;height:15px}",
    "#" + ID + "-root.is-open{left:0;right:0;bottom:0;max-width:none;align-items:stretch}",
    "#" + ID + "-root .ta-panel{width:100%;max-height:88vh;border-radius:20px 20px 0 0;",
    ...
    var root = el(
      '<div id="' + ID + '-root" aria-live="polite">' +
        '<div class="ta-panel" role="dialog" aria-label="Check affordability" aria-hidden="true">' +
          '<div class="ta-head">' +
            '<div class="ta-ico" style="width:28px;height:28px">' +
