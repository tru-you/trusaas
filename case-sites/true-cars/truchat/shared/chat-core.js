...
(function (root) {
  ...
  function injectCss() {
    ...
    s.textContent = [
      /* tokens */
      ".tc-root{--tc-red:#1466E0;--tc-red-dark:#0B4DB0;--tc-rgb:20,102,224;--tc-gold:#ffffff;--tc-gold-dim:rgba(255,255,255,.08);",
      "--tc-bg:#08090f;--tc-panel:rgba(16,18,28,.92);--tc-border:rgba(255,255,255,.1);--tc-text:#f8fafc;",
      "--tc-muted:#94a3b8;--tc-green:#34d399;--tc-user:rgba(var(--tc-rgb),.2);",
      ...
      "background:linear-gradient(165deg,rgba(10,16,28,.95) 0%,rgba(12,14,22,.97) 42%,rgba(10,12,18,.98) 100%);",
      "border:1px solid rgba(255,255,255,.12);border-radius:20px;overflow:hidden;",
      "box-shadow:0 28px 80px -24px rgba(0,0,0,.75),0 0 0 1px rgba(var(--tc-rgb),.08),inset 0 1px 0 rgba(255,255,255,.08);",
      ...
      "background:radial-gradient(ellipse 90% 50% at 50% -10%,rgba(var(--tc-rgb),.22),transparent 55%),",
      "radial-gradient(ellipse 40% 30% at 100% 100%,rgba(21,199,192,.07),transparent 50%)}",
      ...
      "border:1.5px solid rgba(var(--tc-rgb),.45);animation:tcRing 2.4s ease-out infinite}",
      ...
      "box-shadow:0 8px 20px -6px rgba(var(--tc-rgb),.65);overflow:hidden}",
      ...
      ".tc-name em{font-style:normal;background:linear-gradient(90deg,#fff 20%,#9CC4FF 100%);",
      ...
      ".tc-msg.user{align-self:flex-end;background:linear-gradient(145deg,rgba(var(--tc-rgb),.35),rgba(var(--tc-rgb),.18));",
      "border:1px solid rgba(var(--tc-rgb),.4);border-bottom-right-radius:6px;",
      "box-shadow:0 6px 18px -10px rgba(var(--tc-rgb),.5);animation:tcMsgUser .38s var(--tc-ease) both}",
      ...
      ".tc-car:hover{border-color:rgba(var(--tc-rgb),.4);transform:translateY(-2px);",
      "box-shadow:0 12px 28px -14px rgba(var(--tc-rgb),.35)}",
      ...
      "box-shadow:0 6px 16px -8px rgba(var(--tc-rgb),.7)}",
      ...
      ".tc-grid button.sel{border-color:var(--tc-red);background:rgba(var(--tc-rgb),.22);box-shadow:0 0 0 1px rgba(var(--tc-rgb),.2)}",
      ...
      ".tc-composer input:focus{border-color:rgba(var(--tc-rgb),.55);background:rgba(0,0,0,.5);",
      "box-shadow:0 0 0 4px rgba(var(--tc-rgb),.14)}",
      ...
      "box-shadow:0 8px 20px -8px rgba(var(--tc-rgb),.75);",
      "transition:transform .2s var(--tc-spring),filter .15s,box-shadow .2s}",
      ".tc-composer button:hover{filter:brightness(1.1);transform:scale(1.04);box-shadow:0 10px 24px -8px rgba(var(--tc-rgb),.85)}",
      ...
      ".tc-foot span{color:rgba(var(--tc-rgb),.7);font-weight:700}",
      ...
      "letter-spacing:.08em;text-transform:uppercase;color:#9CC4FF;",
      "background:rgba(var(--tc-rgb),.12);border:1px solid rgba(var(--tc-rgb),.3);padding:5px 10px;border-radius:999px;margin-bottom:6px}",
    ].join("");
  ...
  function mount(container, options) {
    ...
    if (CFG.brandRed || CFG.brandPrimary) {
      ...
      var hexm = /^#?([0-9a-f]{6})$/i.exec(String(primary).trim());
      if (hexm) {
        var num = parseInt(hexm[1], 16);
        container.style.setProperty("--tc-rgb", ((num >> 16) & 255) + "," + ((num >> 8) & 255) + "," + (num & 255));
      }
    }
    container.innerHTML =
      '<div class="tc-shell">' +
      '<div class="tc-head">' +
      '<div class="tc-who">' +
      '<div class="tc-avatar-wrap"><div class="tc-avatar" id="tcAv">' + esc((CFG.assistantName || "A").charAt(0)) + "</div></div>" +
      ...
      (options.showFoot !== false
        ? '<div class="tc-foot">' + esc(CFG.brandLine || CFG.dealerName || "Showroom") + " · <span>TruChat</span></div>"
        : "") +
      "</div>";
    ...
    function isOpenHours() {
      ...
      if (day >= 1 && day <= 5) return mins >= 480 && mins <= 1020;
      ...
      (vehicles || []).forEach(function (v) {
        ...
        card.querySelectorAll("button").forEach(function (btn) {
          btn.onclick = function () {
            handleOut(bot.selectVehicleAction(label, btn.getAttribute("data-a")));
          };
        });
    ...
    function handleOut(out) {
      ...
      persistLead(false);
      var qualified = bot.getSession() && bot.getSession().qualified;
      if (out.handoffReady || qualified) {
    ...
    /* ---- AI mode (CFG.aiEndpoint) — Claude-backed replies with scripted fallback ---- */
    var aiHistory = [];
    var aiEnabled = !!CFG.aiEndpoint;
    /* chips whose scripted UX (catalog cards, schedulers, instant handoff) beats free text */
    var SCRIPTED_FIRST = /^(browse stock|browse other stock|trade-?in( valuation)?|book test drive|test drive this car|talk to a human|showroom hours|finance help)$/i;

    function captureIdentity(t) {
      var s = bot.getSession();
      var ph = t.match(/(?:\+27|0)\d{9}/);
      if (ph) { s.phone = ph[0]; bot.recomputeQualified(); }
      var nm = t.match(/(?:my name is|i am|i'm|call me)\s+([a-zA-Z\s]{2,20})/i);
      if (nm) s.name = nm[1].trim();
    }

    function compactStock() {
      var list = (bot.config.catalog || []).slice(0, 20);
      return list.map(function (v) {
        return (v.year || "") + " " + (v.brand || "") + " " + (v.model || "") +
          " — R" + Number(v.price || 0).toLocaleString("en-ZA") +
          (v.km ? ", " + v.km : "") + (v.fuel ? ", " + v.fuel : "");
      });
    }
    ...
    function aiSend(t) {
      ...
      fetch(CFG.aiEndpoint, {
        ...
        body: JSON.stringify({
          messages: aiHistory.slice(-24),
          vehicle: pageVehicle || null,
          stock: compactStock(),
        }),
      })
        ...
        .then(function (d) {
          ...
          addBotText(reply, d.handoff ? ["Continue on WhatsApp"] : []);
          persistLead(false);
          if (d.handoff || (bot.getSession() && bot.getSession().qualified)) {
            waBar.classList.add("show");
          }
        })
        .catch(function () {
          if (destroyed) return;
          setTyping(false);
          aiHistory.pop(); /* scripted brain takes this turn instead */
    ...
    function userSay(text) {
      ...
      if (tl === "continue on whatsapp" || tl === "open handoff" || tl === "open whatsapp") {
        persistLead(true);
      ...
      captureIdentity(t);
      setTyping(true);
      if (aiEnabled && !SCRIPTED_FIRST.test(t)) {
        aiSend(t);
        return;
      }
    ...
    async function loadStock() {
      /* Prefer the live page's own inventory (always current on true-cars) */
      try {
        var tv = (root.TCSA && root.TCSA.vehicles) || [];
        if (tv.length) {
          bot.setCatalog(
            tv.slice(0, 14).map(function (v) {
              return {
                id: v.id,
                brand: String(v.make || "").toUpperCase(),
                model: (v.model || "") + (v.variant ? " " + v.variant : ""),
                year: v.year,
                price: v.price,
                km: (typeof v.km === "number" ? v.km.toLocaleString("en-ZA") : v.km) + " km",
                fuel: v.fuel || "",
              };
            })
          );
