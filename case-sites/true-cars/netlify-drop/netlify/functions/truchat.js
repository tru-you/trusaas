 ...
 * Keeps the Anthropic API key server-side; the widget POSTs the conversation here.
 *
...
- Inspections: every car has a TruVIR condition report (AI-graded from a guided TruLens photo capture) and a Tru3D 360-degree orbit view on its vehicle page.
...
- For dealers: this whole showroom runs on TruSaaS dealer software (the search, Tru3D, pricing badges, this chat). Dealers can book a live walkthrough at www.tru-saas.com.
...
exports.handler = async (event) => {
  ...
  if (!key) return { statusCode: 500, headers: cors, body: JSON.stringify({ error: "ANTHROPIC_API_KEY not configured" }) };
