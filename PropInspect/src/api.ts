...
export async function updateVehicle(id: string, updates: Partial<Property>): Promise<Property> {
  ...
  let updatedV;
  await updateState(s => {
    const idx = s.properties.findIndex(v => v.id === id);
    if (idx !== -1) {
      s.properties[idx] = { ...s.properties[idx], ...updates };
      updatedV = s.properties[idx];
    }
  });
...
export async function updateReconTask(propertyId: string, taskId: string, updates: Partial<Property['maintenanceTasks'][0]>) {
  const state = await fetchState();
  const v = state.properties.find(x => x.id === propertyId);
 ...
 * this path scored exactly 50. The server's POST /api/enquiries has always honoured
 * req.body.digitalScore, so the two paths disagreed.
 */
export async function createLead(
  lead: Omit<Enquiry, "id" | "createdAt" | "lastContactedAt" | "status" | "assignedUserId" | "digitalScore">
    & { digitalScore?: number },
): Promise<Enquiry> {
  try {
    ...
    if (res.ok) {
      const body = await res.json();
      if (body.lead) return body.lead as Enquiry;
    }
  } catch (err) {
  ...
  await updateState(s => s.enquiries.push(newL));
  return newL;
}

export async function updateLead(id: string, updates: Partial<Enquiry>): Promise<Enquiry> {
  // Persist to server first — a local-only push here used to get silently
  ...
  let updated;
  await updateState(s => {
    const idx = s.enquiries.findIndex(l => l.id === id);
    if (idx !== -1) {
      s.enquiries[idx] = { ...s.enquiries[idx], ...updates };
      updated = s.enquiries[idx];
    }
  });
...
export async function updateLeadStatus(
...
): Promise<{ lead: Enquiry; coupledVehicle: { id: string; status: string } | null }> {
  const res = await authFetch(`/api/enquiries/${id}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
  ...
  const body = await res.json();
  return { lead: body.lead as Enquiry, coupledVehicle: body.coupledVehicle ?? null };
}
 ...
 *  drops them out of the billable count; their enquiries and notes stay. */
...
export async function deleteLead(id: string): Promise<void> {
  ...
  await updateState(s => { s.enquiries = s.enquiries.filter(l => l.id !== id); });
}

/**
 * Round-robin unassigned "New" enquiries across the active salespeople.
 * Goes through authFetch — calling /api/enquiries/auto-assign with a bare fetch
...
export async function autoAssignLeads(): Promise<{ message: string; assignments: any[] }> {
  const res = await authFetch("/api/enquiries/auto-assign", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  });
  const data = await res.json().catch(() => ({}));
