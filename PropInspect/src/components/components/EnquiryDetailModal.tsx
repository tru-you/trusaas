...
` +
      `Thanks for your interest in the ${property ? propertyLabel(property) : "property"}. ` +
...
` +
    ...
    sms: () =>
      `Hi ${lead.firstName}, it's ${currentUserName || "the agency"} — the ` +
      `${property ? propertyLabel(property) : "home"} you asked about is still available. Want to come take a look?`.slice(0, 160),
    whatsapp: () =>
      `Hi ${lead.firstName} 👋 The ${property ? propertyLabel(property) : "property"} ` +
  ...
  const [docTypeToGenerate, setDocTypeToGenerate] = useState<"Offer to Purchase" | "Finance Application" | "Property Sale">("Offer to Purchase");
  ...
  const startContactAction = (channel: "call" | "whatsapp" | "email") => {
    if (channel === "email") {
      const subj = property ? `Re: ${propertyLabel(property)}` : "";
    ...
    fetchState()
      .then((state: any) => {
        ...
        if (found) {
          ...
          setWhatsappHistory([
            { sender: "customer", text: `Hi there, I saw your ad for the ${propertyLabel(properties.find(v => v.id === found.propertyId))}. Is it still available?`, time: "Yesterday 14:15" },
            { sender: "agent", text: "Good day! Yes, it is currently in our Sandton showroom. Are you available for a viewing Saturday morning?", time: "Yesterday 14:22" },
  ...
  const property = properties.find((v) => v.id === lead.propertyId);
  ...
  const handleTemplateChange = (val: string) => {
    ...
    if (val === "welcome") {
      setEmailSubject(`Showroom Enquiry Verified — ${propertyName}`);
      setEmailBody(
        `Hi ${lead.firstName},\n\nThank you for reaching out regarding the ${propertyName} (Listing: ${property.listingRef}) listed at R ${property.askingPrice.toLocaleString()}.\n\nThis property is currently available for inspection on our Sandton showroom floor. Are you available for a viewing Saturday morning?\n\nKind regards,\nSandton Estates`
      );
      setSmsBody(`Hi ${lead.firstName}, following up on your ${propertyName} inquiry. Let us know if you want to book a viewing! JHB Estates.`);
      setWhatsappBody(`Good day ${lead.firstName}. Hope you are well. Regarding the ${propertyName}, let me know when is best to touch base to schedule a viewing session!`);
    } else if (val === "followup") {
      setEmailSubject(`Financing Quote Options — ${propertyName}`);
      setEmailBody(
        `Hi ${lead.firstName},\n\nFollowing up on our brief pricing discussion. I have prepared several pre-approval finance options for the ${propertyName}.\n\nLet me know if we can compile your credit documentation for formal submission to our finance houses.\n\nWarm regards,\nSandton Estates`
      );
      setSmsBody(`Hi ${lead.firstName}, finance quotes are prepared for the ${propertyName}. Check your email or Whatsapp details. Thanks!`);
      setWhatsappBody(`Good day ${lead.firstName}. Just a quick check-in. The monthly installment amortization metrics for the ${propertyName} are drafted. Let me know when you can chat.`);
    } else if (val === "finance") {
      setEmailSubject(`Agency Purchase Order Contract Draft — ${lead.lastName}`);
      setEmailBody(
        `Hi ${lead.firstName},\n\nFantastic news! Your credit application terms are pre-approved. I have drafted the Purchase and Sale Contract folder inside our central agency system portal.\n\nPlease review and let me know if you would like me to assist with digital signature sign-off.\n\nRegards,\nSandton Estates`
      );
      setSmsBody(`Good news ${lead.firstName}! Your credit limits are approved. Review the sale contract at your convenience. JHB Estates.`);
  ...
  const handleDispatchComm = async (e: React.FormEvent) => {
    ...
    if (commChannel === "email") {
      subject = emailSubject || "Agency Update Notification";
      content = emailBody;
  ...
  const handleSaveStatus = async () => {
    try {
      /* The home moves with the deal, but the server owns that rule — it is the
      ...
      alert(
        coupledVehicle?.status === "SOLD"
          ? "Deal closed — lead marked Closed Won and the property marked Sold."
          : coupledVehicle?.status === "INVENTORY"
          ? "Lead reopened — the property has been returned to portfolio."
          : "Lead stage updated.",
      );
      ...
      <div className="bg-[color:var(--ink-2)] border border-[rgba(138,162,184,0.22)] rounded-2xl w-full max-w-[720px] max-h-[90vh] overflow-y-auto flex flex-col shadow-2xl relative font-sans animate-in zoom-in-95 duration-150">
        ...
        <div className="p-6 flex-1 overflow-y-auto min-h-0 flex flex-col gap-4">
              ...
              {property && (
                <div className="card !bg-[color:var(--glass)]">
                  <div className="card-body p-4 flex items-center justify-between">
                    <div>
                      <div className="text-[length:var(--t-micro)] font-medium text-[color:var(--muted)] tracking-normal font-mono mb-1">Subject Property Focus</div>
                      <div className="text-[16px] font-semibold text-[color:var(--white)]">{propertyLabel(property)}</div>
                      <div className="text-[13px] text-[rgba(232,234,230,0.72)] mt-0.5">Listing No: {property.listingRef} / Price: R {property.askingPrice.toLocaleString()}</div>
                    </div>
                    <span className="px-2 py-1 bg-[color:var(--cyan-faint)] text-[color:var(--cyan)] text-[13px] font-semibold tracking-normal rounded">
                      {property.status === "INVENTORY" ? "Active Showroom" : "Transferred"}
                    </span>
                  </div>
                </div>
              )}

              {/* Agency Assist qualification — surfaces only on brand-new
                  enquiries with no open tasks. Once anyone has started working
                ...
                <div className="card-body p-5 flex flex-col gap-4 relative overflow-hidden">
                  ...
                  <div className="flex items-center justify-between border-b border-white/5 pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <span className="text-[13px] font-semibold text-[color:var(--cyan)] tracking-normal font-mono block">Agency Assist qualification</span>
                  ...
                  <div className="text-[13px] text-[color:var(--white)] leading-relaxed font-sans space-y-2">
                    ...
                    <p className="text-[rgba(232,234,230,0.72)]">
                      {lead.digitalScore >= 75 
                        ? `Incoming message demonstrates urgent buying intent for the ${property ? propertyLabel(property) : 'property'}. Customer completed virtual financing calculations and viewed showroom photos 5+ times in past 2 hours.`
                        : lead.digitalScore >= 50
                        ? `Moderate interest. Customer is comparing alternatives for the ${property ? propertyLabel(property) : 'property'}. Visited pricing and specifications directories and requested dynamic financing brochure updates.`
                        : `Early research phase. Customer is scanning multiple homes. Subscribed to general newsletters but hasn't performed high-value commitment actions yet.`}
                    </p>
                  </div>

                  {/* Suggested Smart Actions Grid */}
                  <div className="space-y-2 mt-1">
                    <span className="text-[length:var(--t-micro)] font-medium text-[color:var(--muted)] tracking-normal font-mono block">Agency Assist suggestions:</span>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      
                      {/* Suggestion 1: Book Viewing / Call Now */}
                      <button
                        type="button"
                        onClick={() => {
                          setActiveTab("tasks");
                          setNewTaskTitle(`Call now to book viewing for ${property ? propertyLabel(property) : 'property'}`);
                      ...
                      >
                        ...
                        <span className="text-[13px] font-semibold text-[color:var(--white)] block mt-0.5 group-hover:text-[color:var(--cyan)]">Book Viewing</span>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="flex flex-col gap-1">
                    ...
                    >
                      ...
                      <option className="bg-[color:var(--ink-2)]" value="welcome">Welcome Introduction</option>
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          const encodedText = encodeURIComponent(whatsappBody || "Good day! Following up from Sandton Estates.");
          ...
          {activeTab === "finance" && (() => {
            ...
            const estMonthly = property ? Math.round(property.askingPrice * 0.0195) : 0;
            return (
              <div className="flex flex-col gap-5 animate-in fade-in duration-150">
                {/* Deal readiness — status of the steps to close and hand over this
                    deal. Agencies raise invoices/contracts in their own systems; we
                ...
                {/* Repayment estimate — indicative, seeded from this deal's property.
                  ...
                  <div className="card-body p-4 flex flex-col gap-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-black/30 border border-white/5 p-3 rounded-xl flex flex-col">
                        <span className="text-[13px] text-[rgba(232,234,230,0.72)] font-semibold">Property price</span>
                        <span className="text-[13px] font-mono font-semibold text-[color:var(--cyan)]">{property ? `R ${property.askingPrice.toLocaleString()}` : "N/A"}</span>
                      </div>
                      <div className="bg-black/30 border border-white/5 p-3 rounded-xl flex flex-col">
        ...
        <div className="px-4 sm:px-6 py-4 border-t border-[rgba(138,162,184,0.1)] flex flex-col-reverse sm:flex-row gap-3 sm:justify-between sm:items-center bg-[color:var(--glass)]">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            ...
            >
              ...
              <option className="bg-[color:var(--ink-2)]" value="Viewing Scheduled">Viewing Scheduled</option>
