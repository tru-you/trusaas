...
/** Agency identity editor — the fields quoted on invoices, agreements, and
 *  the public listing. Self-service so a agency can correct a VAT number or
 *  swap a trading name without waiting on admin. Admins targeting another
 *  agency pass isAdmin so the server-side check accepts the agencyId
 *  in the body. */
export default function AgencyDetailsSettings({ agency, isAdmin, onSaved }: Props) {
  const [form, setForm] = useState({
    name: agency.name || "",
    tradingAs: agency.tradingAs || "",
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const updated = await updateAgencySelf(form, isAdmin ? agency.id : undefined);
      onSaved?.(updated);
      setSaved(true);
    } catch (err) {
    ...
    <div className="card border-[color:var(--cyan-soft)]">
      <div className="card-header border-b border-white/5 px-5 py-3 flex items-center gap-2">
        ...
        <h3 className="font-semibold text-[16px] text-[color:var(--white)]">
          Agency details
          {isAdmin && <span className="ml-2 text-[12px] text-[rgba(232,234,230,0.55)] font-normal">— {agency.name}</span>}
        </h3>
