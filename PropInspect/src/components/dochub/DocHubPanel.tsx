...
export default function DocHubPanel({ Enquiry, dealership, onLeadRefresh }: Props) {
  ...
  /* `docStage: null` means two opposite things — never started, or finished the
     ...
     at offer. When complete, every stage is behind us. */
  const isComplete = !!Enquiry.docFlowCompletedAt;
  const currentStage = Enquiry.docStage ?? DOC_STAGES[0];
  ...
  const reload = async () => {
    try {
      const res = await authFetch(`/api/deals/${Enquiry.id}/documents`);
  ...
  const patchChecklist = async (patch: Partial<NonNullable<Enquiry["dealChecklist"]>>) => {
    await updateLead(Enquiry.id, {
      dealChecklist: { ...(Enquiry.dealChecklist || {}), ...patch } as Enquiry["dealChecklist"],
    });
    onLeadRefresh?.();
  };
      ...
      <div className="flex items-center gap-2 text-sm text-[rgba(232,234,230,0.72)]">
        ...
        <span>
          ...
          <span className={`font-semibold ${isComplete ? "text-emerald-300" : "text-[color:var(--white)]"}`}>
            {isComplete
              ? `Complete — handed over ${new Date(Enquiry.docFlowCompletedAt!).toLocaleDateString("en-ZA")}`
              : Enquiry.docStage
              ? STAGE_LABEL[Enquiry.docStage]
              : "Not started"}
          </span>
        </span>
      </div>
