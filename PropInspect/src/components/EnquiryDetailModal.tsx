...
` +
    ...
    fetchState()
      .then((state: any) => {
        ...
        if (found) {
          ...
          setWhatsappHistory([
            { sender: "customer", text: `Hi there, I saw your ad for the ${properties.find(v => v.id === found.propertyId)?.make || 'vehicle'}. Is it still available?`, time: "Yesterday 14:15" },
  ...
  const vehicle = properties.find((v) => v.id === lead.propertyId);
      ...
      <div className="bg-[color:var(--ink-2)] border border-[rgba(138,162,184,0.22)] rounded-2xl w-full max-w-[720px] max-h-[90vh] overflow-y-auto flex flex-col shadow-2xl relative font-sans animate-in zoom-in-95 duration-150">
        ...
        <div className="p-6 flex-1 overflow-y-auto min-h-0 flex flex-col gap-4">
                ...
                <div className="card !bg-[color:var(--glass)]">
                  <div className="card-body p-4 flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-[13px] text-[rgba(232,234,230,0.72)] mt-0.5">Stock No: {vehicle.stockNumber} / Price: R {vehicle.askingPrice.toLocaleString()}</div>
                    </div>
          ...
          {activeTab === "finance" && (() => {
            ...
            const estMonthly = vehicle ? Math.round(vehicle.askingPrice * 0.0195) : 0;
            return (
                  ...
                  <div className="card-body p-4 flex flex-col gap-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-black/30 border border-white/5 p-3 rounded-xl flex flex-col">
                        ...
                        <span className="text-[13px] font-mono font-semibold text-[color:var(--cyan)]">{vehicle ? `R ${vehicle.askingPrice.toLocaleString()}` : "N/A"}</span>
                      </div>
                      <div className="bg-black/30 border border-white/5 p-3 rounded-xl flex flex-col">
