...
/**
 * Onboarding a agency, as a screen rather than four curl commands.
 *
 * The endpoints have existed since agencies became data, but nothing in the
 * UI called them, so adding a agency meant hand-writing an admin token into a
...
type Agency = {
  ...
  slug: string;
  websiteUrl?: string;
  /** Which apps this agency's code opens. */
  products?: string[];
  address?: string;
...
/** The apps a agency can be signed up for.
 *
 *  Every product verifies agency codes against this instance, so ticking a box
 *  here is the whole of granting access. It used to mean pasting the agency's
 *  code into that app's environment variable in plaintext and restarting it —
 *  once per app, per agency — and revoking meant editing the same string and
...
const PRODUCT_OPTIONS: Array<{ id: string; label: string; hint: string }> = [
  ...
  { id: "flow", label: "PropInspect", hint: "The DMS — listing, enquiries, invoicing" },
  { id: "flow-lite", label: "PropInspect", hint: "Lightweight agency console — enquiries, portfolio & upload" },
  { id: "inspect", label: "TruInspect", hint: "Condition report / Inspection" },
  { id: "live", label: "TruLive", hint: "Live video walkaround" },
  { id: "value", label: "TruValue", hint: "Live video home-sale appraisal" },
  { id: "social", label: "TruSocial", hint: "Auto-publish listing to social channels" },
];
...
export default function AgencyAdmin({
  ...
  /* Defaults to the common case — a yard buying the DMS and the capture app.
     A Lens-only agency is now a matter of unticking PropInspect, rather than
  ...
  const deleteAgency = async (d: Agency) => {
    ...
    setDeleting(d.id);
    try {
      const res = await authFetch(`/api/agencies/${d.id}`, { method: "DELETE" });
      const body = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(body?.error || `Server responded ${res.status}`);
      setRows((prev) => prev.filter((r) => r.id !== d.id));
      onNotify("Agency removed", body?.message || `${d.name} deleted.`);
    } catch (err: any) {
  ...
  const updateProducts = async (d: Agency, next: string[]) => {
    setSavingProductsFor(d.id);
    try {
      const res = await authFetch(`/api/agencies/${d.id}`, {
        method: "PUT",
      ...
      const body = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(body?.error || `Server responded ${res.status}`);
      setRows((prev) => prev.map((r) => (r.id === d.id ? { ...r, products: body.agency.products } : r)));
  ...
  /* Which agencies already have a login code, and how much listing each is
     actually publishing. Onboarding used to be four steps with no way to see
     which of them you had done — you found out a agency was half set up when
  ...
  const [listingCount, setListingCount] = useState<Record<string, number | null>>({});
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const list: Agency[] = await res.json();
      setRows(list);

      /* Best-effort. A failure here must not blank the agency list, so
      ...
      authFetch("/api/auth/codes")
        ...
        .then((body) => {
          ...
          for (const a of body.accounts) if (a.agencyId) map[a.agencyId] = true;
          setHasCode(map);
        })
        .catch(() => { /* leave unknown */ });

      // The public feed is what the agency's website sees, so ask it the same
      // way the website does rather than counting rows in the DMS.
      list.forEach((d) => {
        fetch(`/api/public/listings?agency=${encodeURIComponent(d.slug)}`, { cache: "no-store" })
          .then((r) => (r.ok ? r.json() : null))
          .then((body) => setListingCount((p) => ({ ...p, [d.id]: typeof body?.count === "number" ? body.count : null })))
          .catch(() => setListingCount((p) => ({ ...p, [d.id]: null })));
      });
    } catch (err: any) {
  ...
  const addAgency = async () => {
    ...
    try {
      ...
      onNotify(
        "Agency added",
        `${body.agency.name} can sign in to ` +
          `${(body.agency.products || []).join(", ") || "no apps"} as soon as they have a code. ` +
          `No environment variable, no redeploy.`,
      );
    } catch (err: any) {
      onNotify("Could not add agency", err?.message || "Unknown error", "error");
    } finally {
      setSaving(false);
    }
  };

  const issueCode = async (d: Agency) => {
    setIssuing(d.id);
    try {
      const res = await authFetch("/api/auth/codes/rotate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ agencyId: d.id }),
      });
      const body = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(body?.error || `Server responded ${res.status}`);
  ...
  /* Step 3 of onboarding is a Render env var, and getting it wrong is the
     failure that files a agency's homes into someone else's yard.
     Real codes are substituted for any agency whose code was issued in this
    ...
    <div className="card border-[color:var(--cyan-soft)]">
      <div className="card-header border-b border-white/5 px-5 py-3 flex items-center justify-between gap-3">
        <h3 className="font-semibold text-[16px] text-[color:var(--white)] flex items-center gap-2">
          <Building2 size={14} className="text-[color:var(--cyan-bright)]" /> Agencies
        </h3>
        <button
          type="button"
          onClick={load}
      ...
      <div className="card-body p-5 flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-3">
          {loading && rows.length === 0 && (
            <p className="text-[13px] text-[rgba(232,234,230,0.72)]">Loading agencies…</p>
          )}

          {rows.length > 0 && (
            <p className="text-[13px] text-[rgba(232,234,230,0.55)]">
              {rows.length} agency{rows.length === 1 ? "" : "s"}
          ...
          {rows.filter((d) => {
            ...
            >
              ...
              <div className="flex items-center gap-3 flex-wrap text-[13px]">
                ...
                <span
                  className={
                    listingCount[d.id] ? "text-[color:var(--cyan)]" : "text-[color:var(--muted)]"
                  }
                >
                  {listingCount[d.id] ? "✓" : "○"}{" "}
                  {listingCount[d.id] === null || listingCount[d.id] === undefined
                    ? "Listing unknown"
                    : listingCount[d.id] === 0
                    ? "No listing live yet"
                    : `${listingCount[d.id]} live on site`}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div className="text-[13px] text-[rgba(232,234,230,0.72)] font-mono break-all min-w-0">
                  {FEED_ORIGIN}/api/public/listings?agency={d.slug}
                </div>
                <CopyButton value={`${FEED_ORIGIN}/api/public/listings?agency=${d.slug}`} label="Copy feed URL" />
              </div>
        ...
        <div className="rounded-xl border border-white/10 p-4 flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2">
            <span className="text-[13px] font-semibold text-[rgba(232,234,230,0.72)] tracking-wider">
              Apps this agency can sign in to
            </span>
          ...
          <p className="text-[13px] text-[rgba(232,234,230,0.72)] leading-relaxed">
            The slug is the agency's identity in their website feed, the TruLens picker and the tag on
            every home they photograph. It <b className="text-[color:var(--white)]">cannot be changed
            later</b> — renaming one orphans their listing, so check it before saving.
          </p>

          {name.trim() && !slugValid && (
            <p className="text-[13px] text-[color:var(--white)]">
              Slug must be lowercase letters, numbers and single hyphens — e.g.{" "}
              <span className="font-mono">homes-on-caledon</span>.
            </p>
          )}
          {slugTaken && (
            <p className="text-[13px] text-[color:var(--white)]">
              <span className="font-mono">{effectiveSlug}</span> is already taken by another agency.
            </p>
          )}
        ...
        {rows.length > 0 && (
          <div className="rounded-xl border border-white/10 p-4 flex flex-col gap-2">
            ...
            <p className="text-[13px] text-[rgba(232,234,230,0.72)] leading-relaxed">
              A agency added here shows up in the TruLens picker straight away. To pin a phone to
              a agency server-side — so a wrong tap cannot file homes into another yard — set{" "}
            ...
            <p className="text-[13px] text-[color:var(--muted)] leading-relaxed">
              {codesLineIsComplete
                ...
                : "Codes issued in this session are filled in below. Any that still read CODE belong to a agency whose code was issued earlier — a code can only be read once, so either use the one you saved or issue a fresh one above."}
            </p>
            <div className="flex items-center justify-between gap-3 flex-wrap">
