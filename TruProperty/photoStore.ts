 ...
 * data.json. One property with a full 22-shot capture makes its record 3.4 MB
 * and 2.9 seconds; readState() parses the whole file — every photo of every
 * property — on each of 58 request paths; and writeState() serialises it all
 ...
 *   - the same photo pushed twice is stored once, which matters because TrueState
 *     re-exports the entire capture every time an inspector adds one more shot;
 ...
 *  stock photos destined for public websites, and the path is an opaque
