...
(function () {
  ...
  var VARS = [
    ...
    "--tf-grad:linear-gradient(135deg,", toHex(accentBright), " 0%,", toHex(accent), " 50%,", toHex(accent2), " 100%);",
    "--tf-ink:#0D131C;--tf-wa:#8C3D12;--tf-wa-deep:#E31B23;",
    "--tf-glass:rgba(18,24,36,.94);--tf-border:rgba(227,27,35,.32);",
    "--tf-text:#FAF5EE;--tf-muted:#A8A29E;--tf-faint:rgba(250,245,238,.45);",
    "--tf-surface:#0D131C;--tf-err:#f87171;",
    "--tf-ease:cubic-bezier(.16,1,.3,1);--tf-spring:cubic-bezier(.34,1.4,.64,1);",
    "--tf-fill:rgba(255,255,255,.05);--tf-fill-2:rgba(255,255,255,.09);",
    "--tf-hair:rgba(227,27,35,.2);--tf-edge:rgba(227,27,35,.35);",
    cfg.theme === "light" ? [
      "--tf-glass:rgba(255,255,255,.94);--tf-border:rgba(227,27,35,.22);",
      "--tf-text:#1C1917;--tf-muted:#57534E;--tf-faint:rgba(28,25,23,.42);",
      "--tf-surface:#FAF5EE;--tf-err:#B91C1C;",
      "--tf-fill:rgba(227,27,35,.04);--tf-fill-2:rgba(227,27,35,.08);",
      "--tf-hair:rgba(227,27,35,.14);--tf-edge:rgba(227,27,35,.24);"
    ].join("") : "",
    (function () {
  ...
  var CSS = [
    ...
    isInline ? "display:block;width:100%;" : [
      ...
      "transition:bottom .35s var(--tf-ease);"
    ].join(""),
    "}",
    isInline ? "" : ":host(.is-open),:host(:has(#tf.is-open)){bottom:20px!important;max-height:calc(100vh - 40px)!important;max-height:calc(100dvh - 40px)!important}",
    "#tf{",
    VARS,
    "font-family:Inter,system-ui,-apple-system,sans-serif;box-sizing:border-box;",
    isInline
    ...
    "box-shadow:0 16px 42px -10px rgba(0,0,0,.65),0 0 24px -6px rgb(var(--tf-signal-rgb)/.35),inset 0 1.5px 1px rgba(255,255,255,.38),inset 0 -1px 1px rgba(0,0,0,.45);",
    "backdrop-filter:blur(28px) saturate(200%);-webkit-backdrop-filter:blur(28px) saturate(200%);",
    "transition:transform .35s var(--tf-spring),box-shadow .35s var(--tf-ease),border-color .25s,background .25s;text-align:left}",
    ".tf-launcher::after{content:'';position:absolute;top:0;left:0;width:45%;height:100%;",
      "background:linear-gradient(90deg,transparent,rgba(255,255,255,.08),rgba(255,255,255,.32),rgba(255,255,255,.08),transparent);",
      "transform:translateX(-160%) skewX(-20deg);animation:tfFloatShine 5s ease-in-out infinite;pointer-events:none}",
    "@keyframes tfFloatShine{0%,35%{transform:translateX(-160%) skewX(-20deg)}65%,100%{transform:translateX(320%) skewX(-20deg)}}",
    ".tf-launcher:hover{transform:translateY(-4px) scale(1.02);background:linear-gradient(135deg,rgba(255,255,255,.24) 0%,rgba(32,40,56,.55) 45%,rgba(13,19,28,.72) 100%);border-color:rgba(255,255,255,.48);",
    "box-shadow:0 22px 52px -10px rgba(0,0,0,.75),0 0 32px -4px rgb(var(--tf-signal-rgb)/.55),inset 0 2px 1.5px rgba(255,255,255,.55),inset 0 -1px 1px rgba(0,0,0,.5)}",
    ".tf-launcher:active{transform:scale(.97)}",
    ".tf-ico{",
    "width:42px;height:42px;border-radius:50%;flex-shrink:0;",
    "background:var(--tf-grad);position:relative;z-index:1;",
    "display:grid;place-items:center;box-shadow:0 6px 18px -3px rgb(var(--tf-signal-rgb)/.7),inset 0 1px 1px rgba(255,255,255,.45)}",
    ".tf-ico::after{content:'';position:absolute;inset:-4px;border-radius:50%;",
    "border:2px solid rgb(var(--tf-signal-rgb)/.45);animation:tfPulse 2.4s ease-out infinite}",
    "@keyframes tfPulse{0%{transform:scale(1);opacity:.65}100%{transform:scale(1.45);opacity:0}}",
    ".tf-ico svg{width:20px;height:20px}",
    ".tf-ltitle{display:block;font-size:13px;font-weight:700;line-height:1.2;letter-spacing:.01em;color:#fff;position:relative;z-index:1}",
    ".tf-lsub{display:block;font-size:10px;color:var(--tf-muted);line-height:1.35;margin-top:2px;position:relative;z-index:1}",
    ...
    isInline
      ...
      : [
          ...
          "border:1px solid rgba(255,255,255,.18);color:var(--tf-text);",
          "backdrop-filter:blur(32px) saturate(1.9);-webkit-backdrop-filter:blur(32px) saturate(1.9);",
          "box-shadow:0 32px 80px -20px rgba(0,0,0,.8),0 0 40px -12px rgb(var(--tf-signal-rgb)/.3),inset 0 1.5px 1px rgba(255,255,255,.2);",
          "opacity:0;transform:translateY(16px) scale(.96);transition:opacity .35s var(--tf-ease),transform .4s var(--tf-spring);"
        ].join(""),
    "}",
    ...
    /* header */
    ".tf-head{",
    "padding:16px 16px 14px;display:flex;align-items:center;gap:12px;",
    "background:var(--tf-grad);position:relative;overflow:hidden;flex-shrink:0;border-bottom:1px solid rgba(255,255,255,.15)}",
    ...
    "animation:tfHeadShine 4s ease-in-out infinite}",
    "@keyframes tfHeadShine{0%,100%{transform:translateX(-120%)}50%{transform:translateX(120%)}}",
    ".tf-head-icon{width:42px;height:42px;border-radius:14px;",
    "background:rgba(255,255,255,.18);border:1px solid rgba(255,255,255,.25);display:grid;place-items:center;flex-shrink:0;position:relative;z-index:1}",
    ".tf-head-icon svg{width:22px;height:22px}",
    ".tf-head b{display:block;font-size:16px;font-weight:800;letter-spacing:-.01em;color:#fff;position:relative;z-index:1}",
    ".tf-head span{display:block;font-size:11.5px;opacity:.9;margin-top:2px;color:#fff;position:relative;z-index:1}",
    ".tf-x{",
    ...
    "transition:background .2s,transform .2s;position:relative;z-index:2}",
    ".tf-x:hover{background:rgba(255,255,255,.28);transform:scale(1.05)}",

    /* body */
    ".tf-body{padding:18px;overflow-y:auto;flex:1;min-height:0}",
    ...
    isInline ? "" : [
      ...
      isInline ? "" : ".tf-launcher{display:none!important}",
      isInline ? "" : ":host{left:0!important;right:0!important;bottom:0!important;width:100%!important;max-width:100%!important;pointer-events:none}",
      isInline ? "" : ":host(.is-open),:host(:has(#tf.is-open)){pointer-events:auto;z-index:999995!important}",
      isInline ? "" : "#tf{width:100%;align-items:stretch}",
      isInline ? "" : ".tf-panel{width:100%!important;max-width:100%!important;border-radius:22px 22px 0 0!important;border-bottom:none!important;max-height:min(88vh,680px)!important;box-shadow:0 -16px 48px rgba(0,0,0,.85)!important;animation:tfSheetIn .35s cubic-bezier(.22,1,.36,1) both!important}",
      "@keyframes tfSheetIn{from{transform:translateY(100%)}to{transform:none}}",
      "}"
    ].join(""),
  ...
  function mount() {
    ...
    var formHtml = [
      ...
      '<div class="tf-panel" role="dialog" aria-modal="', isInline ? 'false' : 'true', '" aria-label="', esc(cfg.heading), '">',
        ...
        '<div class="tf-foot">Powered by <a href="https://tru-saas.com" target="_blank" rel="noopener" style="color:inherit;text-decoration:underline;font-weight:700;">TruDealer™</a></div>',
      '</div>',
      '</div>'
    ].join('');
    ...
    function open(payload) {
      ...
      if (!isInline && shadow && shadow.host) {
        shadow.host.classList.add("is-open");
        shadow.host.style.bottom = "20px";
      }
