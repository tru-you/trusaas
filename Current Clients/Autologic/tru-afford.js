...
(function () {
  ...
  var cfg = {
    ...
    apiKey: attr("data-key", "") || attr("data-api-key", "") || attr("data-dms-key", ""),
  ...
  var VARS = [
    ...
    "--ta-grad:linear-gradient(135deg,", toHex(accentBright), " 0%,", toHex(accent), " 50%,", toHex(accent2), " 100%);",
    "--ta-ink:#0D131C;--ta-ok:#22C55E;--ta-warn:#F59E0B;--ta-warn-text:#fcd34d;",
    "--ta-wa:#8C3D12;--ta-wa-deep:#E31B23;",
    "--ta-glass:rgba(18,24,36,.94);--ta-border:rgba(227,27,35,.32);",
    "--ta-text:#FAF5EE;--ta-muted:#A8A29E;--ta-faint:rgba(250,245,238,.45);",
    "--ta-surface:#0D131C;",
    "--ta-ease:cubic-bezier(.16,1,.3,1);--ta-spring:cubic-bezier(.34,1.4,.64,1);"
  ].join("");
  ...
  var CSS = [
    ...
    "transition:bottom .35s var(--ta-ease)}",
    ":host(.is-open),:host(:has(#" + ID + "-root.is-open)){bottom:20px!important;max-height:calc(100vh - 40px)!important;max-height:calc(100dvh - 40px)!important}",
    "#" + ID + "-root{",
    VARS,
    "font-family:Inter,system-ui,-apple-system,sans-serif;box-sizing:border-box;",
    ...
    "#" + ID + "-root .ta-launcher{",
    "pointer-events:auto;display:flex;align-items:center;gap:12px;padding:10px 18px 10px 12px;position:relative;overflow:hidden;",
    ...
    "box-shadow:0 16px 42px -10px rgba(0,0,0,.65),0 0 24px -6px rgb(var(--ta-signal-rgb)/.35),inset 0 1.5px 1px rgba(255,255,255,.38),inset 0 -1px 1px rgba(0,0,0,.45);",
    "backdrop-filter:blur(28px) saturate(200%);-webkit-backdrop-filter:blur(28px) saturate(200%);",
    "transition:transform .35s var(--ta-spring),box-shadow .35s var(--ta-ease),border-color .25s,background .25s;text-align:left}",
    "#" + ID + "-root .ta-launcher::after{content:'';position:absolute;top:0;left:0;width:45%;height:100%;",
    "background:linear-gradient(90deg,transparent,rgba(255,255,255,.08),rgba(255,255,255,.32),rgba(255,255,255,.08),transparent);",
    "transform:translateX(-160%) skewX(-20deg);animation:taFloatShine 5s ease-in-out infinite;pointer-events:none}",
    "@keyframes taFloatShine{0%,35%{transform:translateX(-160%) skewX(-20deg)}65%,100%{transform:translateX(320%) skewX(-20deg)}}",
    "#" + ID + "-root .ta-launcher:hover{transform:translateY(-4px) scale(1.02);background:linear-gradient(135deg,rgba(255,255,255,.24) 0%,rgba(32,40,56,.55) 45%,rgba(13,19,28,.72) 100%);border-color:rgba(255,255,255,.48);",
    "box-shadow:0 22px 52px -10px rgba(0,0,0,.75),0 0 32px -4px rgb(var(--ta-signal-rgb)/.55),inset 0 2px 1.5px rgba(255,255,255,.55),inset 0 -1px 1px rgba(0,0,0,.5)}",
    "#" + ID + "-root .ta-launcher:active{transform:scale(.97)}",
    "#" + ID + "-root .ta-ico{",
    "width:42px;height:42px;border-radius:50%;flex-shrink:0;",
    "background:var(--ta-grad);position:relative;z-index:1;",
    "display:grid;place-items:center;box-shadow:0 6px 18px -3px rgb(var(--ta-signal-rgb)/.7),inset 0 1px 1px rgba(255,255,255,.45)}",
    "#" + ID + "-root .ta-ico::after{content:'';position:absolute;inset:-4px;border-radius:50%;",
    "border:2px solid rgb(var(--ta-signal-rgb)/.45);animation:taPulse 2.4s ease-out infinite}",
    "@keyframes taPulse{0%{transform:scale(1);opacity:.65}100%{transform:scale(1.45);opacity:0}}",
    "#" + ID + "-root .ta-ico svg{width:20px;height:20px}",
    "#" + ID + "-root .ta-ltitle{display:block;font-size:13px;font-weight:700;line-height:1.2;letter-spacing:.01em;color:#fff;position:relative;z-index:1}",
    "#" + ID + "-root .ta-lsub{display:block;font-size:10px;color:var(--ta-muted);line-height:1.35;margin-top:2px;position:relative;z-index:1}",
    "#" + ID + "-root .ta-badge{display:none}",
    ...
    "max-height:min(calc(100vh - 40px),calc(100dvh - 40px),620px);border-radius:22px;overflow:hidden;",
    "background:linear-gradient(165deg,rgba(22,28,40,.85) 0%,rgba(13,19,28,.92) 100%);",
    "backdrop-filter:blur(32px) saturate(1.9);-webkit-backdrop-filter:blur(32px) saturate(1.9);",
    "border:1px solid rgba(255,255,255,.18);color:var(--ta-text);",
    "box-shadow:0 32px 80px -20px rgba(0,0,0,.8),0 0 40px -12px rgb(var(--ta-signal-rgb)/.3),inset 0 1.5px 1px rgba(255,255,255,.2);",
    ...
    "#" + ID + "-root .ta-head{",
    "padding:16px 16px 14px;display:flex;align-items:center;gap:12px;position:relative;overflow:hidden;",
    "background:var(--ta-grad);",
    "border-bottom:1px solid rgba(255,255,255,.15);flex-shrink:0}",
    "#" + ID + "-root .ta-head::after{content:'';position:absolute;inset:0;background:linear-gradient(120deg,transparent 30%,rgba(255,255,255,.14) 50%,transparent 70%);transform:translateX(-120%);animation:taHeadShine 4s ease-in-out infinite;pointer-events:none}",
    "@keyframes taHeadShine{0%,100%{transform:translateX(-120%)}50%{transform:translateX(120%)}}",
    "#" + ID + "-root .ta-head b{display:block;font-size:15px;font-weight:800;color:#fff;position:relative;z-index:1}",
    "#" + ID + "-root .ta-head span{font-size:11.5px;opacity:.9;color:#fff;position:relative;z-index:1}",
    "#" + ID + "-root .ta-x{",
    "margin-left:auto;width:34px;height:34px;border-radius:10px;background:rgba(255,255,255,.16);",
    "color:#fff;font-size:18px;display:grid;place-items:center;transition:background .2s,transform .2s;position:relative;z-index:1}",
    "#" + ID + "-root .ta-x:hover{background:rgba(255,255,255,.28);transform:scale(1.05)}",
    "#" + ID + "-root .ta-steps{display:flex;gap:6px;padding:12px 16px 0;flex-shrink:0}",
    "#" + ID + "-root .ta-step-dot{flex:1;height:4px;border-radius:4px;background:rgba(255,255,255,.1);transition:background .3s}",
    "#" + ID + "-root .ta-step-dot.on{background:var(--ta-grad)}",
    "#" + ID + "-root .ta-body{padding:16px;overflow-y:auto;flex:1;min-height:0}",
    ...
    "#" + ID + "-root .ta-h{font-size:17px;font-weight:800;letter-spacing:-.02em;margin:0 0 6px;line-height:1.25;color:var(--ta-text)}",
    ...
    "background:rgb(var(--ta-signal-rgb)/.12);border:1px solid rgb(var(--ta-signal-rgb)/.32);",
    "font-size:11.5px;color:var(--ta-signal-bright);line-height:1.45}",
    "#" + ID + "-root .ta-soft svg{width:16px;height:16px;flex-shrink:0;margin-top:1px;stroke:var(--ta-signal-bright)}",
    ...
    "#" + ID + "-root .ta-field{margin-bottom:16px}",
    "#" + ID + "-root .ta-row{display:flex;justify-content:space-between;align-items:center;margin-bottom:6px}",
    "#" + ID + "-root .ta-row output{font-weight:800;font-size:13px;color:var(--ta-signal-bright);font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace}",
    "#" + ID + "-root input[type=range]{width:100%;appearance:none;height:6px;border-radius:100px;outline:none;cursor:pointer;",
    "background:linear-gradient(90deg,var(--ta-signal) 0%,var(--ta-blue) var(--fill,50%),rgba(255,255,255,.1) var(--fill,50%))}",
    "#" + ID + "-root input[type=range]::-webkit-slider-thumb{appearance:none;width:22px;height:22px;border-radius:50%;",
    "background:#fff;border:3px solid var(--ta-signal);box-shadow:0 2px 10px rgb(var(--ta-signal-rgb)/.5);cursor:pointer;transition:transform .2s var(--ta-spring),box-shadow .2s}",
    "#" + ID + "-root input[type=range]:hover::-webkit-slider-thumb{transform:scale(1.18);box-shadow:0 2px 14px rgb(var(--ta-signal-rgb)/.85)}",
    "#" + ID + "-root input[type=text],#" + ID + "-root input[type=tel],#" + ID + "-root select{",
    "width:100%;padding:12px 14px;border-radius:12px;border:1.5px solid rgba(255,255,255,.14);",
    "background:rgba(255,255,255,.05);color:var(--ta-text);font:600 14px Inter,system-ui,sans-serif;outline:none;",
    "transition:border-color .2s,box-shadow .2s}",
    "#" + ID + "-root input:focus,#" + ID + "-root select:focus{",
    "border-color:rgb(var(--ta-signal-rgb)/.7);box-shadow:0 0 0 3px rgb(var(--ta-signal-rgb)/.22)}",
    ...
    /* Mobile: suppress floating launcher on mobile since trigger is in sticky footer; sheet slides up when open */
    "@media (max-width:768px){",
    ":host:not(.is-open):not(.ta-inline){display:none!important}",
    ":host:not(.ta-inline){left:0!important;right:0!important;bottom:0!important;width:100%!important;max-width:100%!important}",
    "#" + ID + "-root .ta-launcher{display:none!important}",
    "#" + ID + "-root:not(.ta-inline){width:100%;align-items:stretch}",
    "#" + ID + "-root .ta-panel{width:100%!important;max-width:100%!important;border-radius:22px 22px 0 0!important;border-bottom:none!important;max-height:min(88vh,680px)!important;box-shadow:0 -16px 48px rgba(0,0,0,.85)!important;animation:taSheetIn .35s cubic-bezier(.22,1,.36,1) both!important}",
    "@keyframes taSheetIn{from{transform:translateY(100%)}to{transform:none}}",
    "}",
  ...
  function mount() {
    ...
    var root = el(
      '<div id="' + ID + '-root" data-app="truafford" aria-live="polite">' +
        '<div class="ta-panel" role="dialog" aria-label="Check affordability" aria-hidden="true">' +
          ...
          '<div class="ta-foot">Powered by <a href="https://tru-saas.com" target="_blank" rel="noopener" style="color:inherit;text-decoration:underline;font-weight:700;">TruDealer™</a> · Soft estimate only · Not a credit bureau check</div>' +
        "</div>" +
        '<button type="button" class="ta-launcher" id="' + ID + '-open" aria-label="Check affordability">' +
          ...
          "<span>" +
            ...
            '<span class="ta-lsub">' + esc(cfg.dealer) + '</span>' +
          "</span>" +
        "</button>" +
      "</div>"
    );
    ...
    if (mountSelector) { root.classList.add("ta-inline", "is-open"); state.open = true; }
  ...
  function open() {
    ...
    if (shadow && shadow.host) {
      shadow.host.classList.add("is-open");
      if (!shadow.host.classList.contains("ta-inline")) shadow.host.style.bottom = "20px";
    }
  ...
  function close() {
    ...
    if (shadow && shadow.host) {
      shadow.host.classList.remove("is-open");
      if (!shadow.host.classList.contains("ta-inline")) shadow.host.style.bottom = "";
    }
      ...
      wa.addEventListener("click", function () {
        ...
        if (cfg.webhook || (cfg.slug && cfg.flowUrl)) {
          ...
          try {
            ...
            if (cfg.apiKey) {
              reqHeaders["x-api-key"] = cfg.apiKey;
