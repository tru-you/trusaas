...
(function (root) {
  ...
  function injectStyle() {
    ...
    s.textContent = [
      ".tb-bg{position:fixed;inset:0;background:", T.scrim, ";backdrop-filter:blur(12px) saturate(1.4);-webkit-backdrop-filter:blur(12px) saturate(1.4);z-index:100005;display:flex;align-items:center;justify-content:center;padding:16px;opacity:0;pointer-events:none;transition:opacity 0.25s ease}",
      ".tb-bg.open{opacity:1;pointer-events:auto}",
      ".tb-modal{background:", T.panel, ";backdrop-filter:blur(28px) saturate(1.8);-webkit-backdrop-filter:blur(28px) saturate(1.8);border:1px solid ", T.edge, ";border-radius:20px;width:92vw;max-width:440px;max-height:min(calc(100vh - 40px),calc(100dvh - 40px),620px);overflow-y:auto;padding:22px 20px 18px;color:", T.ink, ";box-shadow:", T.shadow, ";font-family:Inter,system-ui,sans-serif;box-sizing:border-box !important;animation:tbModalIn 0.3s cubic-bezier(0.16,1,0.3,1) both}",
      ".tb-modal *, .tb-modal *::before, .tb-modal *::after{box-sizing:border-box !important}",
      "@keyframes tbModalIn{from{opacity:0;transform:translateY(12px) scale(0.97)}to{opacity:1;transform:translateY(0) scale(1)}}",
      ".tb-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:8px}",
      ".tb-head h3{margin:0;font-size:18px;font-weight:800;letter-spacing:-0.02em;color:", T.ink, ";font-family:inherit}",
      ".tb-close{background:", T.fill, ";border:1px solid ", T.edge, ";color:", T.mute, ";border-radius:10px;width:32px;height:32px;font-size:18px;cursor:pointer;display:grid;place-items:center;transition:all 0.2s}",
      ".tb-close:hover{color:", T.ink, ";background:", T.fill2, "}",
      ".tb-sub{font-size:12.5px;color:", T.quiet, ";line-height:1.4;margin-bottom:14px}",
      ".tb-modes{display:flex;gap:6px;margin-bottom:14px;overflow-x:auto;padding-bottom:2px}",
      ".tb-mode{padding:8px 14px;border-radius:10px;background:", T.fill, ";border:1px solid ", T.edge, ";font-size:12px;font-weight:600;color:", T.quiet, ";cursor:pointer;white-space:nowrap;transition:all 0.2s}",
      ".tb-mode.on{background:", cfg.accent, ";color:", T.onAcc, ";border-color:transparent;box-shadow:0 4px 14px -3px ", cfg.accent, "80}",
      ".tb-label{font-size:11px;font-weight:700;letter-spacing:0.06em;text-transform:uppercase;color:", T.mute, ";margin:12px 0 6px}",
      ".tb-grid{display:grid;gap:6px}",
      "#tbDays{grid-template-columns:repeat(3,1fr)}",
      "#tbTimes{grid-template-columns:repeat(4,1fr)}",
      ".tb-slot{padding:8px 4px;border-radius:10px;background:", T.fill, ";border:1px solid ", T.edge, ";text-align:center;cursor:pointer;font-size:12px;font-weight:600;color:", T.ink, ";transition:all 0.2s;user-select:none;-webkit-user-select:none}",
      ".tb-slot .day{font-size:10px;font-weight:700;color:", T.mute, ";margin-bottom:2px;text-transform:uppercase;letter-spacing:0.04em}",
      ".tb-slot.on{background:", cfg.accent, ";color:", T.onAcc, ";border-color:transparent;box-shadow:0 4px 14px -2px ", cfg.accent, "60}",
      ".tb-slot.on .day{color:", T.onAcc, ";opacity:.85}",
      ".tb-fields{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px}",
      ".tb-fields input{width:100%;padding:10px 12px;border-radius:10px;background:", T.fill, ";border:1px solid ", T.edge, ";color:", T.ink, ";font-size:13px;font-family:inherit;outline:none;transition:border-color 0.2s,box-shadow 0.2s}",
      ".tb-fields input::placeholder{color:", T.mute, "}",
      ".tb-fields input:focus{border-color:", cfg.accent, ";box-shadow:0 0 0 3px ", cfg.accent, "30}",
      ".tb-fields input.err{border-color:#B91C1C}",
      ".tb-when{margin-top:10px;padding:10px 12px;border-radius:10px;background:", T.fill, ";border:1px solid ", T.edge, ";font-size:12px;color:", T.quiet, ";display:flex;align-items:center;gap:8px}",
      ".tb-btn{width:100%;margin-top:14px;padding:13px 16px;border-radius:12px;background:", cfg.accent, ";color:", T.onAcc, ";border:none;font-weight:700;font-size:14px;cursor:pointer;box-shadow:0 6px 20px -3px ", cfg.accent, "80;transition:all 0.2s}",
      ".tb-btn:hover{transform:translateY(-1px);filter:brightness(1.08)}",
      ".tb-btn:active{transform:scale(0.98)}",
      ".tb-foot{text-align:center;font-size:11px;color:", T.mute, ";margin-top:12px}",
      "@media (max-width:768px){",
      "  .tb-bg{padding:0;align-items:flex-end}",
      "  .tb-modal{width:100%;max-width:100%;border-radius:22px 22px 0 0;border-bottom:none;padding:18px 16px calc(18px + env(safe-area-inset-bottom,0px));max-height:88vh;animation:tbSheetIn 0.32s cubic-bezier(0.22,1,0.36,1) both}",
      "  @keyframes tbSheetIn{from{opacity:0;transform:translateY(100%)}to{opacity:1;transform:translateY(0)}}",
      "  .tb-fields{grid-template-columns:1fr}",
      "  .tb-slot{min-height:42px;display:flex;flex-direction:column;justify-content:center;align-items:center}",
      "}"
    ].join("");
