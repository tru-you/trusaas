/* Caledon — TruValue trade-in profiler.
 *
...
(function () {
  ...
  function factorsHtml() {
    return '<div class="ti-factors">'
      ...
      + "<p>We don't let software guess your car's worth. A Caledon buyer walks the car with you on a live TruValue call and gives you a trade price in writing, subject to viewing — a real number from a real person.</p></div>";
  }
  ...
  function summary() {
    ...
    var rows = [
      ...
      ["Mileage", state.km ? Number(String(state.km).replace(/\D/g, "")).toLocaleString("en-ZA") + " km" : "—"],
  ...
  function handoff() {
    ...
    var car = {
      ...
      name: state.model,
      priceLabel: [
        state.km ? Number(String(state.km).replace(/\D/g, "")).toLocaleString("en-ZA") + " km" : "",
    ...
    var text = ["Hi Cars on Caledon — I'd like a TruValue trade-in.", "",
      "Vehicle: " + [state.year, state.make, state.model].filter(Boolean).join(" "),
