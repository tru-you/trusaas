...
export default function DealerAssist({ userName }: Props) {
  ...
  const [messages, setMessages] = useState<ChatMsg[]>([
    { role: "assistant", content: "Hey! I'm your TruInspect assistant. Ask me anything — inspections, checklists, damage reports, trade-ins, or any TruSaaS question." },
  ]);
  const [input, setInput] = useState("");
  const [busy, setBusy] = useState(false);
  const [hint, setHint] = useState(() => !localStorage.getItem("truinspect_assist_seen"));
  ...
  const suggestions = messages.length <= 1
    ? ["How to run an inspection?", "Trade-in appraisal tips", "What goes in the report?"]
    : null;

  return (
    <>
      <button
        onClick={() => { setOpen(true); setHint(false); localStorage.setItem("truinspect_assist_seen", "1"); }}
