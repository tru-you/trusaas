  ...
  return (
    <div className="flex flex-col h-full bg-neutral-950 text-[#E8EAE6] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-24">
        {/* Summary card */}
        <div className="rounded-[18px] bg-[rgba(232,234,230,0.055)] border border-[rgba(232,234,230,0.14)] p-4 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-[14px] font-medium">{capturedCount} of {totalSlots} shots</span>
            {allRequiredDone ? (
              <span className="flex items-center gap-1.5 text-[12px] text-cyan-400">
              ...
              <div
                ...
                className={`rounded-xl px-4 py-3 border flex items-center justify-between ${
                  allDone
                    ...
                    : 'bg-[rgba(232,234,230,0.055)] border-[rgba(232,234,230,0.14)]'
                }`}
              >
                <div>
                  <p className="text-[13px] font-medium">{phase.name}</p>
                  <p className="text-[12px] text-[rgba(232,234,230,0.55)]">{phaseCaptured} of {phaseSlots.length}</p>
                </div>
                {allDone && <Check size={16} className="text-cyan-400" strokeWidth={3} />}
              </div>
            );
          })}
        </div>
