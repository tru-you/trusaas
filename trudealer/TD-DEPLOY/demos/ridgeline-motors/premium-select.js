...
async function tryLoadLive(){
  ...
  const stock=await Ridgeline.loadStock("select");
  if(stock&&stock.length){STOCK=stock;setStockSource(true,STOCK.length+" Select vehicles");}
  else{STOCK=Ridgeline.MOCK.filter(c=>c.cat===CAT);setStockSource(false,STOCK.length+" Select vehicles");}
  applyFilters();
}
