...
async function tryLoadLive(){
  ...
  const stock=await Ridgeline.loadStock("performance");
  if(stock&&stock.length){STOCK=stock;setStockSource(true,STOCK.length+" Performance vehicles");}
  else{STOCK=Ridgeline.MOCK.filter(c=>c.cat===CAT);setStockSource(false,STOCK.length+" Performance vehicles");}
  applyFilters();
}
