...
(function (root) {
  ...
  function mount(container, options) {
    ...
    var bot = TQ.createQualifier({
      ...
      salesText: CFG.personalText || CFG.salesText,
    ...
    container.innerHTML =
      ...
      '<div class="tc-wa-bar" id="tcWaBar"><span>You\'re ready — continue with the yard on Text</span>' +
      '<button type="button" id="tcWaBtn">Open Text</button></div>' +
    ...
    function handleOut(out) {
      ...
      if ((out.handoffReady || (bot.getSession() && bot.getSession().qualified)) && !handoffPrompted) {
        ...
        addBotText(
          "Perfect — you're **qualified**. Tap **Open Text** and the yard gets your full ticket instantly.",
          ["Continue on Text"]
        );
        ...
        try {
          ...
          var mapped = list.slice(0, 12).map(function (v) {
            return {
              ...
              km: (v.mileage || v.km || "") + (v.mileage ? " mi" : ""),
