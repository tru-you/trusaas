/**
 * TruChat qualifier engine — shared by web bot + future Text webhook.
  ...
  function createQualifier(config) {
    ...
    function process(text) {
      ...
      if (phoneMatched) {
        ...
        say(
          "Thanks — I've logged **" +
            ...
            "**. " +
            (session.qualified
              ? "You're all set; our team will follow up on Text shortly."
              : "One of the team will follow up on Text.") +
            (session.qualified ? "\n\n✅ *Lead qualified*" : ""),
      ...
      if (lower.includes("yes") && (lower.includes("finance") || session.financeInterest)) {
        ...
        say(
          "Great — finance can run a soft assessment. Please share your **mobile number** so we can continue on Text.",
          []
        );
        return { replies: replies, session: session };
      }
    ...
    function whatsappHandoffUrl() {
      const phone = String(cfg.salesText || "").replace(/\D/g, "");
