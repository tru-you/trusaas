/**
 * Text Business bot bridge — shared brain path for Ray.
 *
 * Flow:
 *  1. Buyer messages the Text Business number
 ...
 *     so the ticket is sent to Ray's personal Text (not the business line)
 *
...
(function (root) {
  ...
  function createWaBusinessBot(config, deps) {
    ...
    function getOrCreate(sessionKey) {
      ...
      if (!sessions[key]) {
        sessions[key] = TQ({
          ...
          salesText: personalDigits(), // personal = final human handoff
    ...
    /**
     * Process one inbound Text user message.
     * @returns {{
     *   replies: Array,
     *   session: object,
