/* Ridgeline Booking — one calendar for every appointment on the site.
   Replaces the "straight to Text" jump on viewing CTAs with a real slot
   picker: pick a day, pick a time, leave a name and number. Confirmation
   still hands off to Text (that's where the showroom team lives) but the
   customer also gets a calendar (.ics) file for their own diary.

   Public API:  RidgelineBook.open({ mode, car, title })
...
(function () {
  ...
  var WA = "15125550142";
  var ADDRESS = "Ridgeline Motors, 4400 South Congress Ave, Austin, TX 78745";
  ...
  var MODES = {
    showroom: {
      ...
      sub: "Come see it in person at 4400 South Congress Ave. Coffee's on us — no pressure, no hard sell.",
      dur: 60,
      ev: "Showroom visit — Ridgeline Motors",
      loc: ADDRESS
    },
    live: {
      label: "Live video",
      ...
      dur: 30,
      ev: "Live video viewing — Ridgeline Motors",
      loc: "Live video call (link sent by Text)"
    },
    tradein: {
      ...
      dur: 30,
      ev: "TruValue remote trade-in — Ridgeline Motors",
      loc: "Live video call (link sent by Text)"
    }
  };
  ...
  function paint() {
    ...
    bg.innerHTML = '<div class="vd-booking">'
      ...
      + '<div class="vd-booking-note">No charge, no obligation. We confirm on Text within business hours — Mon&ndash;Fri 08:00&ndash;17:30, Sat 08:00&ndash;14:00.</div>'
      + "</div>";
  ...
  // Local-time ICS (no Z suffix) — the slot is United Statesn trading hours.
  function ics(name) {
    ...
    var desc = [
      ...
      "Booked by: " + name,
      "Ridgeline Motors · +1 512 555 0142"
    ].filter(Boolean).join("\\n");
    return [
      "BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Ridgeline Motors//Booking//EN",
      "BEGIN:VEVENT",
      "UID:mkr-" + f(se[0]) + "-" + Math.abs(hashStr(title)) + "@ridgelinemotors.example",
      "DTSTAMP:" + f(new Date()),
      "DTSTART:" + f(se[0]),
      "DTEND:" + f(se[1]),
      "SUMMARY:" + title,
  ...
  function confirm() {
    ...
    var text = [
      "Hi Ridgeline — I'd like to book a " + m.label.toLowerCase() + ".",
      "",
      "When: " + when,
  ...
  /* ---------- intercept the old straight-to-Text CTAs ---------- */
  ...
  function intercept() {
    document.addEventListener("click", function (e) {
      ...
      // Only take over booking-shaped CTAs; plain "Text us" links stay as they are.
  ...
  window.RidgelineBook = { open: open, close: close };
})();
