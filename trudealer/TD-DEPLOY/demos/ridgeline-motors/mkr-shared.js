/* Ridgeline Motors — shared stock, render, chat, FAB helpers */
const Ridgeline = (() => {
  ...
  const MOCK = [
    {y:2023,make:"Nissan",name:"Navara 2.5DDTi PRO-2X",variant:"Double Cab 4×4 Auto",body:"truck",cat:"performance",km:"42 846 km",tr:"Auto",fuel:"Diesel",price:529900,truPrice:559000,tag:"Premium Performance",vir:"4.8",img:"https://images.unsplash.com/photo-1533473359331-0135ef1b58bf?auto=format&fit=crop&w=1000&q=80"},
    {y:2021,make:"Volkswagen",name:"Golf 8 GTI",variant:"DSG · Performance pack",body:"hatch",cat:"performance",km:"38 200 km",tr:"DSG",fuel:"Gas",price:489900,truPrice:514000,tag:"Premium Performance",vir:"4.7",img:"https://images.unsplash.com/photo-1617531653332-bd46c24f2068?auto=format&fit=crop&w=1000&q=80"},
    {y:2018,make:"BMW",name:"M2 Coupé",variant:"Manual · Long Beach Blue",body:"coupe",cat:"performance",km:"64 000 km",tr:"Manual",fuel:"Gas",price:589900,truPrice:625000,tag:"Premium Performance",vir:"4.6",img:"https://images.unsplash.com/photo-1555215695-3004980ad54e?auto=format&fit=crop&w=1000&q=80"},
    {y:2019,make:"Audi",name:"RS3 Sportback",variant:"Quattro · Sports exhaust",body:"hatch",cat:"performance",km:"58 500 km",tr:"S-Tronic",fuel:"Gas",price:549900,truPrice:579000,tag:"Premium Performance",vir:"4.7",img:"https://images.unsplash.com/photo-1606664515524-ed2f786a0bd6?auto=format&fit=crop&w=1000&q=80"},
    {y:2012,make:"MINI",name:"Cooper S Hatch",variant:"Manual · Character",body:"hatch",cat:"performance",km:"174 187 km",tr:"Manual",fuel:"Gas",price:139900,truPrice:148500,tag:"Premium Performance",vir:"4.3",img:"https://images.unsplash.com/photo-1549317661-bd32c8ce0db2?auto=format&fit=crop&w=1000&q=80"},
    {y:2019,make:"Hyundai",name:"Tucson 2.0 Premium",variant:"Auto · Full history",body:"suv",cat:"select",km:"33 711 km",tr:"Auto",fuel:"Gas",price:259900,truPrice:272000,tag:"Premium Select",vir:"4.7",img:"https://images.unsplash.com/photo-1519641471654-76ce0107ad1b?auto=format&fit=crop&w=1000&q=80"},
    {y:2026,make:"Volkswagen",name:"Polo Vivo 1.4 Life",variant:"Hatch · Like new",body:"hatch",cat:"select",km:"3 736 km",tr:"Manual",fuel:"Gas",price:279900,truPrice:298500,tag:"Premium Select",vir:"4.9",img:"https://images.unsplash.com/photo-1541899481282-d53bffe3c35d?auto=format&fit=crop&w=1000&q=80"},
    ...
    {y:2021,make:"Mercedes-Benz",name:"C200 AMG Line",variant:"Auto · Panoramic",body:"sedan",cat:"select",km:"41 200 km",tr:"Auto",fuel:"Gas",price:619900,truPrice:649000,tag:"Premium Select",vir:"4.8",img:"https://images.unsplash.com/photo-1618843479313-40f8afb4b4d8?auto=format&fit=crop&w=1000&q=80"},
    {y:2020,make:"Ford",name:"Ranger Wildtrak 2.0",variant:"Bi-Turbo 4×4 Auto",body:"truck",cat:"select",km:"72 000 km",tr:"Auto",fuel:"Diesel",price:489900,truPrice:519500,tag:"Premium Select",vir:"4.6",img:"https://images.unsplash.com/photo-1533473359331-0135ef1b58bf?auto=format&fit=crop&w=1000&q=80"},
    {y:2025,make:"Hyundai",name:"Grand i10 1.0 Motion",variant:"Hatch",body:"hatch",cat:"used",km:"11 136 km",tr:"Manual",fuel:"Gas",price:219900,truPrice:232000,tag:"Premium Used",vir:"4.8",img:"https://images.unsplash.com/photo-1502877338535-766e1452684a?auto=format&fit=crop&w=1000&q=80"},
    {y:2016,make:"Toyota",name:"Corolla 1.6 Esteem",variant:"Sedan",body:"sedan",cat:"used",km:"91 906 km",tr:"Manual",fuel:"Gas",price:179900,truPrice:189000,tag:"Premium Used",vir:"4.5",img:"https://images.unsplash.com/photo-1621007947382-bb3c3994e3fb?auto=format&fit=crop&w=1000&q=80"}
  ];
  ...
  function mapApi(v) {
    ...
    else if (/truck|truck|4x4|double/.test(bodyRaw)) body = "truck";
    ...
    // (a $520k Corolla reads as "performance" on price alone).
    ...
    return {
      ...
      body,
      cat,
      km: v.mileage != null ? Number(v.mileage).toLocaleString("en-US") + " mi" : (v.km || "—"),
      tr: v.transmission || "—",
  ...
  function renderCards(list, gridEl) {
    if (!gridEl) return;
    if (!list.length) {
      gridEl.innerHTML = '<p class="empty-msg">No vehicles in this category right now — Text us and we\'ll source or notify you.</p>';
      return;
    }
    gridEl.innerHTML = list.map((c, i) => {
      const wa = encodeURIComponent(`Hi Ridgeline, I'm interested in the ${c.y} ${c.make} ${c.name} (${fmtR(c.price)}) — ${c.tag}. Is it still available?`);
      return `<article class="card rv d${(i % 3) + 1}" ${dataAttrs(c)}>
        ...
        <div class="card-body">
          ...
          <div class="cta-row">
            <a class="btn btn-wa" href="https://wa.me/${WA}?text=${wa}" target="_blank" rel="noopener">Text</a>
            <a class="btn btn-ghost-blue" href="https://wa.me/${WA}?text=${encodeURIComponent("Hi Ridgeline, I'd like a viewing of the " + c.y + " " + c.make + " " + c.name)}" target="_blank" rel="noopener">Viewing</a>
          </div>
        </div>
      </article>`;
    }).join("");
  ...
  function initChat(opts = {}) {
    ...
    const replies = {
      buy: "Browse this category grid, then Text a unit or book a viewing / live video.",
    ...
    document.getElementById("chatQuick")?.addEventListener("click", e => {
      ...
      setTimeout(() => {
        ...
        m.className = "msg bot";
        m.textContent = replies[q] || "Text us and we’ll help right away.";
