...
/* ===== STOCK DATA ===== */
let STOCK = Ridgeline.MOCK.slice();
...
function render(list){
  ...
  grid.innerHTML = show.map((c,i)=>{
    ...
    return '<article class="card rv d'+((i%3)+1)+'" data-idx="'+i+'" '+Ridgeline.dataAttrs(c)+'>'
      ...
      +'<a class="btn btn-ghost-blue" href="https://wa.me/'+WA+'?text='+encodeURIComponent("Hi Ridgeline, I\'d like a viewing of the "+c.y+" "+c.make+" "+c.name)+'" target="_blank" rel="noopener" onclick="event.stopPropagation()">Viewing</a>'
      +'</div></div></article>';
  }).join("");
  observeAll();
}
...
async function tryLoadLive(){
  ...
  const stock = await Ridgeline.loadStock();
...
function applyFilters(make,body,maxP,sort){
  ...
  if(!list.length){
    ...
    grid.insertAdjacentHTML("afterbegin",'<p style="grid-column:1/-1;text-align:center;padding:36px 12px;font-size:13px;color:var(--grey);font-weight:600;">No exact matches &mdash; showing featured stock. Text us to source.</p>');
  } else render(list);
}
...
(function(){
  ...
  document.addEventListener("click", e=>{
    ...
    if(btn.classList.contains("chip")){
      ...
      span.style.background="rgba(255,122,47,.25)"; span.style.pointerEvents="none";
...
if(window.Ridgeline_TRUCHAT_CONFIG){
  window.TRUECARS_TRUCHAT_CONFIG = window.Ridgeline_TRUCHAT_CONFIG;
}