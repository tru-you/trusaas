...
function render(list){
  ...
  currentList=show;
  grid.innerHTML=show.map((c,i)=>'<article class="card rv d'+((i%3)+1)+'" data-idx="'+i+'"><div class="card-shine" aria-hidden="true"></div><div class="ph"><div class="im" style="background-image:linear-gradient(180deg,transparent 50%,rgba(10,22,38,.4)),url(\''+(c.img||'')+'\')"></div><span class="badge">Premium Select</span><span class="vir">VIR &#9733; '+(c.vir||"4.5")+'</span></div><div class="card-body"><div class="yr">'+c.y+' &middot; '+c.make+'</div><h3>'+c.name+'</h3><div class="var">'+(c.variant||"")+'</div><div class="specs"><span>'+(c.km||"&mdash;")+'</span><span>'+(c.tr||"&mdash;")+'</span><span>'+(c.fuel||"&mdash;")+'</span><span>'+(c.body||"&mdash;")+'</span></div><div class="price-row"><div class="pr">'+fmtR(c.price)+'</div><div class="pm">from <b>'+fmtR(monthly(c.price))+'/pm</b></div></div>'+MKR.tpTag(c)+'<div class="cta-row"><button class="btn btn-gold" onclick="event.stopPropagation();openVehicleDetail(currentList['+i+'])">View details</button><a class="btn btn-ghost-blue" href="https://wa.me/'+WA+'?text='+encodeURIComponent("Hi MKR, I'd like a viewing of the "+c.y+" "+c.make+" "+c.name)+'" target="_blank" rel="noopener" onclick="event.stopPropagation()">Viewing</a></div></div></article>').join("");
  observeAll();
}

/* LIVE STOCK */
async function tryLoadLive(){
  setStockSource(false,"Loading Select stock...");
  const stock=await MKR.loadStock("select");
  if(stock&&stock.length){STOCK=stock;setStockSource(true,STOCK.length+" Select vehicles");}
  else{STOCK=MKR.MOCK.filter(c=>c.cat===CAT);setStockSource(false,STOCK.length+" Select vehicles");}
  applyFilters();
}
...
function applyFilters(){
  ...
  if(!list.length){render(STOCK.slice(0,6));grid.insertAdjacentHTML("afterbegin",'<p style="grid-column:1/-1;text-align:center;padding:36px 12px;font-size:13px;color:var(--grey);font-weight:600;">No exact matches — showing all Select stock.</p>');}
  else render(list);
}
...
/* INIT */
render(STOCK);
setStockSource(false,STOCK.length+" Select vehicles");
tryLoadLive();
observeAll();
