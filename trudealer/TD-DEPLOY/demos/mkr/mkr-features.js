/* MKR Features — Virtual Trade-In simulator + interactive delivery map
...
(function () {
  ...
  function tradeIn() {
    ...
    if (form) form.addEventListener("submit", function (e) {
      ...
      if (window.MKRBook) {
        window.MKRBook.open({ mode: "tradein", car: theirCar });
        return;
      }
      var text = [
        "Hi MKR — I'd like to do a virtual trade-in.",
        "",
  ...
  function delivery() {
    ...
    function select(g) {
      ...
      if (kmEl) kmEl.innerHTML = Number(km).toLocaleString("en-ZA") + '<em> km</em>';
      if (etaEl) etaEl.innerHTML = eta;
      if (pickEl) pickEl.innerHTML = "Gqeberha to <b>" + name + "</b> &mdash; fully insured in transit, <b>R0</b> delivery fee.";
      if (waEl) waEl.href = "https://wa.me/" + WA + "?text=" +
        encodeURIComponent("Hi MKR, I'm in " + name + " — what does delivery look like for me?");
    }
    ...
    var jhb = section.querySelector('.mkr-city[data-city="Johannesburg"]');
    if (jhb) select(jhb);
  }
