/* Halstead Booking — one calendar for every appointment on the site.
   ...
   Public API:  COCBook.open({ mode, car, title })
...
(function () {
  ...
  var ADDRESS = "Halstead Motor Co., 257 Halstead Street, Manchester";
  ...
  var MODES = {
    showroom: {
      ...
      dur: 60,
      ev: "Showroom visit — Halstead Motor Co.",
      loc: ADDRESS
    },
    live: {
      label: "Live video",
      ...
      dur: 30,
      ev: "Live video viewing — Halstead Motor Co.",
    ...
    tradein: {
      ...
      dur: 30,
      ev: "TruValue remote trade-in — Halstead Motor Co.",
      loc: "Live video call (link sent by WhatsApp)"
    }
  };
  ...
  function paint() {
    ...
    bg.innerHTML = '<div class="vd-booking">'
      ...
      + '<img src="demo-logo.svg" alt="" style="width:34px;height:34px;border-radius:9px;object-fit:cover;flex-shrink:0">'
      + "<h3>" + esc(m.title) + "</h3></div>"
      + '<button class="vd-booking-close" id="cocBkClose" aria-label="Close">&times;</button></div>'
      + '<div class="sub">' + esc(m.sub) + "</div>"
      + carCard()
      + '<div class="coc-bk-modes" id="cocBkModes">'
      + Object.keys(MODES).map(function (k) {
        return '<div class="coc-bk-mode' + (k === mode ? " on" : "") + '" data-mode="' + k + '" role="button" tabindex="0">' + MODES[k].label + "</div>";
      }).join("")
      + "</div>"
      + '<div class="vd-slot-label">Select a day</div>'
      + '<div class="vd-slot-grid" id="cocBkDays">'
      ...
      + '<div class="vd-slot-grid" id="cocBkTimes">' + timeGridHtml(selDay) + "</div>"
      + '<div class="vd-slot-label">Your details</div>'
      + '<div class="coc-bk-fields">'
      + '<input id="cocBkName" placeholder="Your name" autocomplete="name" required>'
      + '<input id="cocBkPhone" placeholder="Mobile number" inputmode="tel" autocomplete="tel" required>'
      + (mode === "tradein" && !car ? '<input class="full" id="cocBkCar" placeholder="Your car — e.g. 2019 Toyota Hilux 2.8, 86 000 km" autocomplete="off">' : "")
      + "</div>"
      + '<div class="coc-bk-when" id="cocBkWhen"></div>'
      + '<button class="vd-booking-confirm" id="cocBkConfirm">Confirm this slot</button>'
      + '<div class="vd-booking-note">No charge, no obligation. We confirm on WhatsApp within business hours — Mon&ndash;Fri 08:00&ndash;17:30, Sat 08:00&ndash;13:00.</div>'
      + "</div>";
  ...
  function paintWhen() {
    var el = document.getElementById("cocBkWhen");
    if (!el) return;
    var s = slots[selDay];
  ...
  function activate(container, el) {
    Array.prototype.forEach.call(container.querySelectorAll(".vd-slot, .coc-bk-mode"), function (x) { x.classList.remove("on"); });
    el.classList.add("on");
  }

  function onPick(container, handler) {
    container.addEventListener("click", function (e) {
      var el = e.target.closest(".vd-slot, .coc-bk-mode");
  ...
  function bind() {
    document.getElementById("cocBkClose").addEventListener("click", close);

    onPick(document.getElementById("cocBkModes"), function (el) {
      mode = el.getAttribute("data-mode");
      paint();
    });

    var days = document.getElementById("cocBkDays");
    var times = document.getElementById("cocBkTimes");
    ...
    document.getElementById("cocBkConfirm").addEventListener("click", confirm);
  }
  ...
  // Local-time ICS (no Z suffix) — the slot is United Kingdomn trading hours.
  function ics(name) {
    ...
    var desc = [
      ...
      "Booked by: " + name,
      "Halstead Motor Co. · +44 161 496 0000"
    ].filter(Boolean).join("\\n");
    return [
      "BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Halstead Motor Co.//Booking//EN",
      "BEGIN:VEVENT",
      // Was "mkr-…@mkrautosales.co.za" — a competitor's domain stamped on every
      // viewing invite Halstead sent, visible in the event properties in Outlook
      // and Google Calendar. The surrounding PRODID and description were already
      // branded correctly; only this line was missed.
      "UID:coc-" + f(se[0]) + "-" + Math.abs(hashStr(title)) + "@halsteadmotors.example",
      "DTSTAMP:" + f(new Date()),
      "DTSTART:" + f(se[0]),
      "DTEND:" + f(se[1]),
      "SUMMARY:" + title,
  ...
  function confirm() {
    var nameEl = document.getElementById("cocBkName");
    var phoneEl = document.getElementById("cocBkPhone");
    var carEl = document.getElementById("cocBkCar");
    ...
    var text = [
      "Hi Halstead Motor Co. — I'd like to book a " + m.label.toLowerCase() + ".",
      "",
      "When: " + when,
    ...
    var icsName = "coc-" + mode + "-" + s.date.getFullYear() + pad(s.date.getMonth() + 1) + pad(s.date.getDate()) + ".ics";

    bg.querySelector(".vd-booking").innerHTML =
      '<div class="coc-bk-done">'
      + '<div class="coc-bk-tick"><svg viewBox="0 0 24 24"><path d="M20 6L9 17l-5-5"/></svg></div>'
      ...
      + '<button class="ics" id="cocBkDone2">Done</button>'
      + "</div></div>";

    var waLink = bg.querySelector(".coc-bk-acts .wa");
    if (waLink) waLink.focus();
    bg.querySelector("#cocBkDone2").addEventListener("click", close);
  }
  ...
  function open(opts) {
    ...
    var first = bg.querySelector(".coc-bk-mode.on");
    ...
    if (window.openBookingModal) {
      ...
      window.openBookingModal = function (c) {
        try {
          open({
            mode: "live",
            car: c && { y: c.y, make: c.make, name: c.name, img: c.img, priceLabel: (window.fmtR || function (n) { return "£" + n; })(c.price) }
          });
        } catch (err) { native(c); }
      };
    }
  ...
  window.COCBook = { open: open, close: close };
})();
