 ...
 * Drop-in: <script src="tru-afford.js" data-dealer="Halstead Motor Co." data-wa="+44 161 496 0000" data-brand="coc"></script>
...
(function () {
  ...
  /* These defaults only apply if the script-tag lookup above returns null.
     ...
     lookup on this site would have quietly sent Halstead's finance enquiries to
     Your Car Guy. A fallback should fail towards the dealer whose site this is. */
  var cfg = {
    dealer: (scr && scr.getAttribute("data-dealer")) || "Halstead Motor Co.",
    wa: (scr && scr.getAttribute("data-wa")) || "+44 161 496 0000",
  ...
  var CSS = [
    ...
    "transition:transform .35s var(--ta-spring),box-shadow .35s var(--ta-ease);max-width:196px;text-align:left}",
    "#" + ID + "-root .ta-launcher:hover{transform:translateY(-3px) scale(1.02);",
    "box-shadow:0 18px 40px -10px rgba(124,58,237,.4),0 0 32px -6px rgba(59,130,246,.3),inset 0 1px 0 rgba(255,255,255,.14)}",
    ...
    "#" + ID + "-root .ta-ltitle{display:block;font-size:12px;font-weight:700;line-height:1.2;letter-spacing:.01em}",
    "#" + ID + "-root .ta-lsub{display:none}",
    ...
    "#" + ID + "-root .ta-panel{",
    "pointer-events:auto;display:none;flex-direction:column;width:min(340px,calc(100vw - 24px));",
    "max-height:min(72vh,560px);border-radius:18px;overflow:hidden;",
    ...
    "box-shadow:0 14px 32px -10px rgba(124,58,237,.55),0 0 24px -8px rgba(59,130,246,.35)}",
    ...
    "background:linear-gradient(120deg,#A78BFA,#60A5FA 55%,#5EEAD4);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}",
