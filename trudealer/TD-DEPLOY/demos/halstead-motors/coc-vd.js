/* Halstead Motor Co. — Vehicle Detail Modal logic (self-contained)
   Tru3D · VIR · TruLens AI · TruPrice · Finance · Booking. Mock data. */
(function(){
var WA = "+44 161 496 0000";
var DEALER = "Halstead Motor Co.";
...
function fmtR(n){return "R"+Number(n||0).toLocaleString("en-GB").replace(/,/g," ");}
...
function phStyle(img, overlay){
  ...
  return "background:radial-gradient(100% 80% at 26% 14%,rgba(46,84,190,.45),transparent 62%),linear-gradient(150deg,#0E1420 0%,#152238 50%,#080A0F 100%)";
}
...
window.openVehicleDetail = function(car){
  ...
  var catClass = car.category==="performance"
    ? "background:linear-gradient(135deg,#F59E0B,#4D9BFF)"
    : car.category==="select"
  ...
  vdContent.innerHTML =
    ...
    +'<div class="vd-vir-eyebrow"><img src="demo-logo.svg" alt="">Verified Inspection Report</div>'
    ...
    +'</div>'
    +'<div class="vd-trust">'
    +'<span><i style="background:#4D9BFF"></i>Available now</span>'
...
window.openCocBooking = function(car){
  ...
  bg.innerHTML='<div class="vd-booking">'
    +'<div class="vd-booking-head"><div style="display:flex;align-items:center;gap:10px"><img src="demo-logo.svg" alt=""><h3>Book a live video viewing</h3></div><span class="vd-booking-close" id="vdBookingClose">&times;</span></div>'
